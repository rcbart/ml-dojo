...
the idea: <b>error is how wrong; loss is how wrong overall; training makes it small.</b></p>`,
 ...
 quiz:{title:'Quick check, the vocabulary',questions:[
   {q:'In machine learning, the "error" of a prediction is:',
    options:['The gap between the prediction and the truth','The time the program takes to run','A bug that crashes the program mid-run','A missing or corrupted data file'],answer:0,whyWrong:['','Runtime is a performance measure. Error is about the answer, not the clock.','A crash is a software fault. A model can run flawlessly and still be wrong.','A bad file is a data problem. It can cause error, and it is not what the word means here.'],
