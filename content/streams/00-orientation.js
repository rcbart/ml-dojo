...
the idea: <b>error is how wrong; loss is how wrong overall; training makes it small.</b></p>`,
 ...
 quiz:{title:'Quick check, the vocabulary',questions:[
   {q:'In machine learning, the "error" of a prediction is:',
    options:['The time the program takes to run','The gap between the prediction and the truth','A bug that crashes the program mid-run','A missing or corrupted data file'],answer:1,whyWrong:['Runtime is a performance measure. Error is about the answer, not the clock.','','A crash is a software fault. A model can run flawlessly and still be wrong.','A bad file is a data problem. It can cause error, and it is not what the word means here.'],
   ...
   {q:'When a later math lesson says "gradient descent reduces the error," it means:',
    options:['It hides the error messages from you','It makes the program run much faster','It fixes bugs in your code','It turns the model\u2019s knobs to shrink the loss'],answer:3,whyWrong:['Nothing is hidden. The number goes down because the model gets better.','Speed is unaffected. The same arithmetic runs at the same rate.','Bugs are yours to fix. Gradient descent only adjusts the weights.',''],
   ...
   {q:'A "loss" or "cost" function is:',
    options:['A single score of how wrong the model is','The number of input features in the dataset','The price you paid for the software licence','A particular kind of program crash or exception'],answer:0,whyWrong:['','That is the feature count, a property of the data rather than of the fit.','Nothing financial is involved. The word is borrowed from optimisation.','An exception is a software fault. A loss is a number the model produces.'],
   ...
   {q:'A program with no bugs at all:',
    options:['Can still have large model error anyway','Must therefore have zero model error','Cannot be trained on any data','Will never produce any predictions'],answer:0,whyWrong:['','Correct code can implement a model that is simply too simple for the pattern.','It trains fine. Whether it trains well is a separate question.','It will produce predictions happily. Whether they are any good is what error measures.'],
