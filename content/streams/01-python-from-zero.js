...
these tools, no libraries, just Python, to prove the ideas stuck.</p>`,
 ...
 quiz:{title:'Quick check, the tools of the trade',questions:[
   ...
   {q:'In the way ML uses the word, the "dimension" of a data point means:',
    options:['How many independent numbers it takes to describe it','How much memory the data point occupies once it is stored','How large the numbers describing it happen to be','How many decimal places its values are recorded to'],
...
and slicing data into batches.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'After total = 10; total += 5; total *= 2, what is total?',
    options:['25, from adding 5 and then adding the original 10 again','30, since 10 + 5 is 15, and 15 times 2 is 30','17, adding 5 and then adding 2','20, doubling the original value of 10'],answer:1,whyWrong:['25 adds 5 and then 10, which is not what the second line does.','','17 is 10 plus 5 plus 2, treating the second operator as addition.','20 doubles the original 10 and skips the addition.'],
    why:'+= updates in place (10 → 15), then *= doubles it (15 → 30). Augmented assignment is just "do the op, then store back."'},
   {q:'17 % 5 evaluates to:',
    options:['3, the number of times 5 divides into 17 completely','3.4, the result of dividing 17 by 5','85, the result of multiplying 17 by 5','2, the remainder after taking out three whole 5s'],answer:3,whyWrong:['3 is how many whole 5s fit, which is 17 // 5.','3.4 is 17 / 5, ordinary division.','85 is 17 times 5.',''],
   ...
   {q:'The expression  "yes" if x > 0 else "no"  is:',
    options:['A function call, with the condition passed in as the first argument','A ternary, a compact if/else that produces one of two values','A loop, repeating while the condition holds','A syntax error, since if cannot appear inside an expression'],answer:1,whyWrong:['There are no parentheses and nothing is being called.','','Nothing repeats. It evaluates once and produces a value.','It is valid Python, and the unusual word order is what makes it look wrong at first.'],
...
several things, which is why Python code leans on it so heavily.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'What does a function return if it never reaches a return statement?',
    options:['None, the "nothing here" value','0, the default numeric result','An empty string, since nothing was produced','It raises an error about a missing return'],answer:0,whyWrong:['','Zero is also a value, and it is not the default.','An empty string is a value you would have to return deliberately.','No error occurs. Falling off the end is perfectly legal.'],
...
= "loop and check." If any confuses you, expand it back into a plain for-loop and it is obvious.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'all(n > 0 for n in [3, 1, -2]) is:',
    options:['An error, since all cannot take a generator expression','False, since minus two is not greater than zero','True, since two of the three numbers are greater than zero','3, the first element that satisfies the condition'],answer:1,whyWrong:['It is valid. Generator expressions are exactly what all expects.','','Minus two fails the test, and all requires every item to pass.','3 is the first element. all returns a truth value.'],
...
<code>for k, v in …</code> unpacks that pair into two handy names.</div>`,
 quiz:{title:'Quick check',questions:[
   ...
   {q:'What does "a b a".split() return?',
    options:['["a", "b", "a"]','"aba"','["a", " ", "b", " ", "a"]','["a b a"]'],answer:0,whyWrong:['','That is what .join would give you, with the spaces removed.','split discards the whitespace it breaks on, so the separators never appear in the result.','A single-element list is what you get from splitting on a separator that never occurs.'],
...
The traceback is just the trail it left while flying.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'The most useful line of a long traceback is usually:',
    options:['A line from the middle, where the failing call was made','None of it. Tracebacks are noise around the real message','The first line, which names the file and line where execution began','The last line, the exception type and message, then work upward'],answer:3,whyWrong:['The middle is the call stack, useful after you know what went wrong.','They are dense and they carry the answer. Reading them is a learnable skill.','The first line is usually the least specific part of the trace.',''],
...
scripts. Read it as "if this file is the one being run, start here."</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'What does "import numpy as np" do?',
    options:['Re-runs the numpy source from scratch every time you use np','Renames the numpy package on disk to np','Downloads numpy from the internet if it is missing','Loads the installed package and gives it the short name np'],answer:3,whyWrong:['The module runs once and is then cached for the rest of the session.','Nothing on disk changes. The alias exists only inside your file.','Downloading is pip\'s job. Import expects it to be there already.',''],
...
mistake impossible.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'Why is "with open(...) as f:" preferred over plain open()?',
    options:['It reads the file faster, by buffering the whole contents into memory at once','The block guarantees the file closes, even if an error is raised inside it','It creates the file when it does not already exist','It is shorter to type than opening and closing the file by hand'],answer:1,whyWrong:['Reading speed is identical.','','Creating a missing file depends on the mode you pass, not on the with-block.','It is slightly longer to type, and the guarantee is what you are paying for.'],
   ...
   {q:'input() returns what the user typed as:',
    options:['Always a string, so you convert it yourself with int or float','A boolean, when the user types something like yes or no','A list of words, split on the spaces the user typed','An integer, whenever what the user typed happens to be all digits'],answer:0,whyWrong:['','A boolean would need converting from the text yourself.','Splitting into words is something you do afterwards, with .split().','It never converts. Digits come back as a string too, which is the usual surprise.'],
   ...
   {q:'A CSV file is, at bottom:',
    options:['Plain text, one line per row with commas between the values','Compressed data, which is why the files stay small','A binary database format that needs a special library to read','An Excel format that other tools can only approximate'],answer:0,whyWrong:['','Nothing is compressed, which is why the files get large.','It is plain text and you can open it in any editor.','Excel reads and writes it, and so does everything else. That is the point of the format.'],
...
ten seconds. That posture is competence.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'"Knowing Python" is best understood as:',
    options:['Fluency with the core, plus the reflexes to find the rest when needed','Never having to reach for the documentation again','Restricting yourself to the handful of functions you were explicitly taught','Having the whole standard library committed to memory'],answer:0,whyWrong:['','Everyone reads documentation, constantly and without embarrassment.','That would leave you stuck at the edge of every lesson.','Nobody has it memorized, and the libraries change faster than anyone could.'],
