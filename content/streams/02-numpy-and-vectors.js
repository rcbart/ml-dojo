...
now you know the ladder is there.</p>`,
 ...
 quiz:{title:'Quick check, did the idea land?',questions:[
   ...
   {q:'"Support Vector Machine", stripped of the intimidating name, is really:',
    options:['A physical machine built to store and retrieve vectors quickly','A method that draws the boundary sitting as far from both groups as it can','A programming language designed for mathematical work','A way of compressing data so it takes less room on disk'],
    answer:1,whyWrong:['Nothing physical is involved. The word machine is a historical quirk of the name.','','It is an algorithm, not a language.','Compression is unrelated. The method fits a boundary.'],
...
you.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'X[:, 1] reads as:',
    options:['The first two rows of the array','Every column except the first one','Every row, column 1, the whole second column','Row 1, the second row of the array'],answer:2,whyWrong:['Two rows would be X[:2]. The comma changes the meaning entirely.','That would be X[:, 1:], with a colon after the index.','','Row 1 is X[1], or X[1, :]. The comma decides which axis you are indexing.'],
...
A memory-free virtual copy plus a compiled loop. That is the entire magic of NumPy.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'Why is vectorized NumPy so much faster than a Python for-loop doing the same math?',
    options:['It caches the result, so repeated calls cost nothing','It moves the work onto the GPU without being asked','It samples half the numbers and estimates the rest','The loop runs in compiled C rather than in the interpreter'],answer:3,whyWrong:['Nothing is cached. It computes the whole answer every time, just far faster.','NumPy runs on the CPU. GPU work needs a different library.','Every number is used. Nothing is skipped.',''],
