...
now you know the ladder is there.</p>`,
 ...
 quiz:{title:'Quick check, did the idea land?',questions:[
   {q:'In the plainest terms, a vector is:',
    options:['An ordered list of numbers, picturable as an arrow in space','A kind of neural network built for sequential data','A database table optimised for numerical columns','A spreadsheet formula that operates across a range of cells'],
    answer:0,whyWrong:['','Networks operate on vectors. The vector itself is just the list of numbers.','A table is a way of storing data. A vector is a mathematical object with arithmetic defined on it.','Formulas compute values. A vector is the thing being computed with.'],
   ...
   {q:'"Support Vector Machine", stripped of the intimidating name, is really:',
    options:['A way of compressing data so it takes less room on disk','A method that draws the boundary sitting as far from both groups as it can','A physical machine built to store and retrieve vectors quickly','A programming language designed for mathematical work'],
    answer:1,whyWrong:['Compression is unrelated. The method fits a boundary.','','Nothing physical is involved. The word machine is a historical quirk of the name.','It is an algorithm, not a language.'],
   ...
   {q:'The dot product of two vectors gives you:',
    options:['Another vector, longer than either of the two inputs','A matrix holding every pairwise product of the entries','The average of the two vectors, entry by entry','A single number measuring how aligned the two are'],
    answer:3,whyWrong:['The result is a single number, so nothing gets longer.','That is the outer product, which builds a grid instead of collapsing to one number.','The average is a vector halfway between them, and it says nothing about alignment.',''],
...
you.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'X[:, 1] reads as:',
    options:['Columns except the first','Row 1','The first two rows','Every row, column 1, the whole second column'],answer:3,whyWrong:['That would be X[:, 1:], with a colon after the index.','Row 1 is X[1], or X[1, :]. The comma decides which axis you are indexing.','Two rows would be X[:2]. The comma changes the meaning entirely.',''],
    why:'Comma = "rows, columns"; ":" = all. So X[:, 1] is column 1 across all rows.'},
   {q:'X[X[:, 1] > 200] returns:',
    options:['The number of rows where column 1 exceeds 200','The rows of X where column 1 exceeds 200','An error, since an array cannot be used as an index','An array of True and False, one value per row'],answer:1,whyWrong:['Counting would be the sum of the mask, or its size.','','It is a standard idiom and works exactly as written.','The mask itself is the True and False values. Indexing with it returns the rows.'],
...
A memory-free virtual copy plus a compiled loop. That is the entire magic of NumPy.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'X is 4×2 and m = X.mean(axis=0) has shape (2,). X - m works because:',
    options:['Broadcasting stretches the two-number row across all four rows','m is collapsed to a single number before the subtraction happens','It does not work, and NumPy raises a shape error','NumPy ignores the shapes and subtracts element by element in order'],answer:0,whyWrong:['','Nothing becomes a scalar. m stays a two-number row and is reused across the rows.','It works, and it is one of the most used idioms in the library.','Shapes are checked carefully. That is exactly what the broadcasting rules are.'],
   ...
   {q:'After Z = (X - X.mean(axis=0)) / X.std(axis=0), each column of Z has:',
    options:['Its original units, since the shift and the scale cancel out','Mean 0 and spread 1, so every feature is on the same ruler','Only positive values, since the division removes the sign','Mean 1 and spread 0, the two statistics the other way round'],answer:1,whyWrong:['The units are divided out. That is what makes columns comparable afterwards.','','Roughly half the values end up negative, since the mean is now zero.','That is the two statistics swapped around.'],
   ...
   {q:'Why is vectorized NumPy so much faster than a Python for-loop doing the same math?',
    options:['The loop runs in compiled C over packed memory rather than in the interpreter','It samples half the numbers and estimates the rest from them','It moves the work onto the GPU without being asked','It caches the result, so repeated calls cost nothing'],answer:0,whyWrong:['','Every number is used. Nothing is skipped.','NumPy runs on the CPU. GPU work needs a different library.','Nothing is cached. It computes the whole answer every time, just far faster.'],
