...
now you know the ladder is there.</p>`,
 ...
 quiz:{title:'Quick check, did the idea land?',questions:[
   {q:'In the plainest terms, a vector is:',
    options:['An ordered list of numbers, picturable as an arrow in space','A spreadsheet formula that operates across a range of cells','A database table optimized for numerical columns','A kind of neural network built for sequential data'],
    answer:0,whyWrong:['','Formulas compute values. A vector is the thing being computed with.','A table is a way of storing data. A vector is a mathematical object with arithmetic defined on it.','Networks operate on vectors. The vector itself is just the list of numbers.'],
   ...
   {q:'"Support Vector Machine", stripped of the intimidating name, is really:',
    options:['A physical machine built to store and retrieve vectors quickly','A method that draws the boundary sitting as far from both groups as it can','A programming language designed for mathematical work on vectors and matrices','A way of compressing data so it takes less room on disk'],
    answer:1,whyWrong:['Nothing physical is involved. The word machine is a historical quirk of the name.','','It is a classification algorithm, not a language. MATLAB and Julia are languages built for that kind of work.','Compression is unrelated. The method fits a boundary.'],
   ...
   {q:'The dot product of two vectors gives you:',
    options:['Another vector, longer than either of the two inputs','The average of the two vectors, entry by entry','A single number measuring how aligned the two are','A matrix holding every pairwise product of the entries'],
    answer:2,whyWrong:['The result is a single number, so nothing gets longer.','The average is a vector halfway between them, and it says nothing about alignment.','','That is the outer product, which builds a grid instead of collapsing to one number.'],
...
you.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'X[:, 1] reads as:',
    options:['The first two rows of the array','Every column except the first, across all of the rows','Every row, column 1, the whole second column','Row 1, the second row of the array'],answer:2,whyWrong:['Two rows would be X[:2]. The comma changes the meaning entirely.','That would be X[:, 1:], with a colon after the index.','','Row 1 is X[1], or X[1, :]. The comma decides which axis you are indexing.'],
    why:'Comma = "rows, columns"; ":" = all. So X[:, 1] is column 1 across all rows.'},
   {q:'X[X[:, 1] > 200] returns:',
    options:['The rows of X where column 1 exceeds 200','An array of True and False, one value per row','An error, since an array cannot be used as an index','The number of rows where column 1 exceeds 200'],answer:0,whyWrong:['','The mask itself is the True and False values. Indexing with it returns the rows.','It is a standard idiom and works exactly as written.','Counting would be the sum of the mask, or its size.'],
   ...
   {q:'X.mean(axis=0) gives:',
    options:['One mean per row, collapsing the columns','The median of every value in the array','A single number, the mean of the whole array','One mean per column, collapsing the rows'],answer:3,whyWrong:['One mean per row is axis=1. The axis names which direction gets collapsed.','The median has its own function. Nothing about the axis changes mean into median.','A single number is X.mean() with no axis at all.',''],
...
A memory-free virtual copy plus a compiled loop. That is the entire magic of NumPy.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'X is 4×2 and m = X.mean(axis=0) has shape (2,). X - m works because:',
    options:['m is collapsed to a single number before the subtraction happens','It does not work, and NumPy raises a shape error','NumPy ignores the shapes and subtracts element by element in order','Broadcasting stretches the two-number row across all four rows'],answer:3,whyWrong:['Nothing becomes a scalar. m stays a two-number row and is reused across the rows.','It works, and it is one of the most used idioms in the library.','Shapes are checked carefully. That is exactly what the broadcasting rules are.',''],
   ...
   {q:'After Z = (X - X.mean(axis=0)) / X.std(axis=0), each column of Z has:',
    options:['Mean 0 and spread 1, so every feature is on the same ruler','Mean 1 and spread 0, the two statistics the other way round','Only positive values, since the division removes the sign','Its original units, since the shift and the scale cancel out'],answer:0,whyWrong:['','That is the two statistics swapped around.','Roughly half the values end up negative, since the mean is now zero.','The units are divided out. That is what makes columns comparable afterwards.'],
   ...
   {q:'Why is vectorized NumPy so much faster than a Python for-loop doing the same math?',
    options:['It caches the result of each element, so repeated calls cost nothing','It moves the work onto the GPU without being asked','It samples half the numbers and estimates the rest','The loop runs in compiled C rather than in the interpreter'],answer:3,whyWrong:['Nothing is cached. It computes the whole answer every time, just far faster.','NumPy runs on the CPU. GPU work needs a different library.','Every number is used. Nothing is skipped.',''],
