...
turns out to be five lines you could have written yourself.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The notation Σ (sum over i from 1 to n) of xᵢ translates to which Python?',
    options:['len(x), which counts how many values there are','The list of x values, written out one after another','max(x), the largest of the values','sum(x), a loop that accumulates a running total'],answer:3,whyWrong:['len(x) counts the items rather than adding them.','The list is what you sum over. Sigma produces one number from it.','max is a different Greek-free operation entirely.',''],
   ...
   {q:'argmax over p of a function means:',
    options:['The average value the function takes across its inputs','The input p at which the function is largest','The number of inputs the function accepts','The largest value the function reaches anywhere'],answer:1,whyWrong:['The average is a different summary and has nothing to do with where the peak is.','','That is a count of inputs, not a choice among them.','That is max. argmax gives you where it happens, not how high it is.'],
   ...
   {q:'The Pi symbol (capital) in a formula tells you to:',
    options:['Multiply the terms together, the way a likelihood is built','Add the terms together, one after another','Divide the result by the constant pi','Take the derivative of the expression that follows'],answer:0,whyWrong:['','Adding is capital sigma. Pi is its multiplicative counterpart.','The constant pi is lowercase and is a different symbol altogether.','Derivatives are written with d or partial signs.'],
...
layers.</p></div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The sigmoid function 1/(1+e^(-x)) is used in ML because it:',
    options:['Squashes any real number into the range 0 to 1','Makes small numbers larger, so gradients do not vanish','Sorts the values so the largest score comes first','Is the fastest activation function to evaluate'],answer:0,whyWrong:['','It bounds the output rather than enlarging it. Large inputs get squashed toward one.','Nothing is sorted. It transforms one number at a time.','It involves an exponential and a division, so speed is not its selling point.'],
   ...
   {q:'Why must the composed functions be nonlinear for depth to help?',
    options:['Composing linear functions collapses back to a single linear function','Linear functions cannot be composed with one another in the first place','It does not matter, since depth adds capacity whatever the activation is','Nonlinear functions are faster to evaluate, so depth becomes affordable'],answer:0,whyWrong:['','They compose perfectly well, and the result is another linear function, which is the problem.','It matters completely. Without it, depth buys you nothing at all.','Speed is not the argument. Expressiveness is.'],
