...
<tr><td>det(A)</td><td>the <b>determinant</b>, the volume-scaling factor (0 ⇒ no inverse)</td></tr>
 ...
 quiz:{title:'Quick check',questions:[
   {q:'A dataset of 200 patients, each described by 5 measurements, is naturally a matrix of shape:',
    options:['(5, 200)','(200, 5)','(200, 200)','(5, 5)'],answer:1,whyWrong:['That is the transpose. The convention is one row per example, so patients come first.','','That would be a patient-by-patient grid, such as a distance matrix, and it holds no measurements.','That is the shape of a covariance matrix over the five measurements, not the data itself.'],
   ...
   {q:'Adding two matrices of the same shape works by:',
    options:['It is not allowed','Multiplying rows by columns','Joining them side by side','Adding matching cells, position by position'],answer:3,whyWrong:['Addition is defined whenever the shapes match. It is multiplication that has the awkward rules.','That is matrix multiplication, and it needs the inner dimensions to agree rather than the whole shape.','Joining side by side is concatenation, which changes the shape rather than combining the values.',''],
   ...
   {q:'Why does ML use matrix operations instead of Python loops?',
    options:['One operation applies the same work to every row, in compiled code','It is a tradition inherited from the numerical computing of the 1970s','Loops over numeric data are disallowed once NumPy has been imported','Matrices use noticeably less memory than the equivalent Python lists'],answer:0,whyWrong:['','Speed is measurable and enormous. Tradition has nothing to do with it.','Loops are legal Python. They are simply far too slow at this scale.','Memory use is comparable. The gain is that the loop moves into compiled code.'],
...
numbers are wrong, check whether you wanted <code>*</code> or <code>@</code>.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'[1, 2, 3] + [4, 5, 6] (as vectors) gives:',
    options:['[1,2,3,4,5,6] (joined)','32','[4, 10, 18]','[5, 7, 9], add matching entries'],answer:3,whyWrong:['That is list concatenation, which is what Python\'s + does to lists rather than to vectors.','32 is the dot product, which sums the products instead of keeping them separate.','That is the element-wise product, not the sum.',''],
   ...
   {q:'In NumPy, a * b (for two equal-length arrays) computes:',
    options:['An error, since the two arrays would first have to be reshaped','Element-wise multiplication, each entry times its partner','The dot product, collapsing both arrays to a single number','Matrix multiplication, treating each array as a single row'],answer:1,whyWrong:['It is perfectly legal, and it is the source of a lot of confusion precisely because it works.','','That is a @ b in NumPy, or np.dot.','Matrix multiplication is @ as well. NumPy\'s * is always element-wise.'],
   ...
   {q:'The difference between element-wise multiply and the dot product is:',
    options:['The dot product also sums the products into one number','Element-wise multiplication is defined only for matrices','The dot product reverses the order of one of the vectors','They are the same operation written two different ways'],answer:0,whyWrong:['','Element-wise works on vectors, matrices and arrays of any shape.','Nothing is flipped. The transpose is a separate operation.','One returns a vector and the other returns a scalar, which is as different as two operations get.'],
...
whole operation.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'In C = A @ B, each cell of C is:',
    options:['The sum of a row of A','The dot product of a row of A with a column of B','Always 0 or 1','The product of matching cells of A and B'],answer:1,whyWrong:['Summing a row ignores B entirely, and both matrices have to contribute.','','The entries are whatever the arithmetic gives. There is no restriction to zero and one.','That is element-wise multiplication, which needs identical shapes and is a different operation.'],
   ...
   {q:'X is 3×2 (3 houses, 2 features) and w has 2 weights. X @ w gives:',
    options:['A 2×3 matrix','2 numbers, one per feature','3 numbers, one prediction per house','An error'],answer:2,whyWrong:['The output has one entry per row of X. A 2x3 matrix is neither the right shape nor the right kind of object.','One number per feature would be the weights themselves, and those are what you started with.','','The shapes line up. 3x2 times 2x1 is valid and gives 3x1.'],
   ...
   {q:'The identity matrix I is special because:',
    options:['I @ v = v, it changes nothing, like multiplying by 1','It doubles every value','It sorts the matrix','It only works on 2×2 matrices'],answer:0,whyWrong:['','Doubling would be 2I. The identity has ones on the diagonal.','Nothing is sorted. Matrix multiplication does not reorder anything.','It exists at every size, one per square shape.'],
...
network needs a nonlinearity between them to bend space at all (calculus/functions stream).</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'What does it mean that "a matrix transforms space"?',
    options:['It measures how much space the data occupies in memory','It moves every point to a new location, reshaping the whole plane','It deletes the points that fall outside a fixed region','It stores a representation of space that can be queried later'],answer:1,whyWrong:['Measuring size is what a determinant or a norm does.','','Points are moved rather than removed. A singular matrix collapses them onto a line, and they are still there.','Storage is a matter of arrays in memory, not of what the transformation means.'],
   ...
   {q:'The columns of a 2×2 matrix tell you:',
    options:['Its inverse, read directly off the entries','Where the basis vectors [1,0] and [0,1] land after the transformation','Its determinant, which is the product of the two columns','How many rows the matrix has, and therefore its shape'],answer:1,whyWrong:['The inverse is computed from the matrix, and it is not read off the columns.','','The determinant is a single number derived from all four entries.','The row count is the shape, which you already know.'],
   ...
   {q:'If a matrix squashes 2-D space flat onto a single line, then:',
    options:['Nothing special happens, since the matrix still maps every point somewhere','Its determinant is huge, because the whole plane is compressed into one place','Area collapses to zero, so the determinant is zero and there is no inverse','It becomes the identity, since the line it maps onto is its own axis'],answer:2,whyWrong:['It is one of the most consequential things that can happen. The transformation stops being reversible.','A huge determinant means a large expansion. Collapse is the opposite, and it gives zero.','','The identity leaves everything where it is, which is as far from collapse as you can get.'],
...
decision, not a formality.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The distance between two data points a and b, as vectors, is:',
    options:['Their average','The number of matching entries','The norm of their difference, ‖a − b‖','a · b'],answer:2,whyWrong:['The average gives you a point between them, not how far apart they are.','Counting matches is a similarity measure for categorical data, and it ignores magnitude entirely.','','The dot product measures alignment. Two identical points have a large dot product and zero distance.'],
   ...
   {q:'Cosine similarity divides the dot product by both lengths. What does that buy?',
    options:['It guarantees a positive answer, whatever the two vectors are','Nothing, since dividing by the lengths leaves the ordering unchanged','Faster computation, because the norms are cheaper than the dot product','Only direction matters, so magnitude stops affecting the comparison'],answer:3,whyWrong:['It can be negative, when the vectors point in opposing directions.','It differs whenever the vectors have different lengths, which is most of the time.','It costs two extra norms, so it is slower rather than faster.',''],
   ...
   {q:'The L1 norm of [3, −4] and the L2 norm of [3, −4] are:',
    options:['5 and 7','7 and 5','1 and 25','Both 7'],answer:1,whyWrong:['Those are the wrong way round and the first is wrong too. L1 is 3 plus 4, L2 is the square root of 9 plus 16.','','1 and 25 are the differences and the sum of squares, neither of which is a norm.','They differ. Seven is L1 and five is L2.'],
...
and up it is more work by hand, but the <i>meaning</i> is identical, a volume-scaling factor.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The determinant of [[3, 1], [2, 4]] is:',
    options:['5, the sum of the two entries on the main diagonal','12, the product of the two entries on the main diagonal','14, the two diagonal products added together','10, the main diagonal product minus the anti-diagonal one'],answer:3,whyWrong:['5 is the sum of the diagonal minus something, and it is not the determinant formula.','12 is just the main diagonal product. You still have to subtract the other one.','14 adds the anti-diagonal instead of subtracting it.',''],
    why:'2×2 determinant is ad − bc = (3)(4) − (1)(2) = 12 − 2 = 10.'},
   {q:'Geometrically, a determinant of 6 means the matrix:',
    options:['Has 6 somewhere among its entries','Has 6 rows, and therefore 6 dimensions','Scales every area or volume by a factor of 6','Adds 6 to each entry of the vector it is applied to'],answer:2,whyWrong:['The entries and the determinant are different things. A matrix with no 6 in it can have determinant 6.','The row count is the shape. The determinant is computed from the values.','','Nothing is added. The determinant is a multiplicative scale factor.'],
   ...
   {q:'A determinant of 0 tells you the matrix:',
    options:['Collapses space onto a lower dimension, so it has no inverse','Doubles every area it touches, in the way a determinant of 2 would','Is very large, since a determinant of zero indicates numerical overflow','Is the identity, which leaves every vector exactly where it started'],answer:0,whyWrong:['','Doubling would be a determinant of 2 in one dimension, or 4 in two.','Size is the shape. A large matrix can have any determinant, including zero.','The identity has determinant 1 and changes nothing.'],
...
un-squashed. That is the whole reason det = 0 ⇔ no inverse.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The defining property of the inverse A⁻¹ is:',
    options:['A @ A⁻¹ = I, so applying then un-applying leaves everything where it was','A⁻¹ is A with its rows written in the opposite order','Every square matrix has one, whatever its entries are','A⁻¹ has negative entries wherever A has positive ones'],answer:0,whyWrong:['','Flipping is the transpose, and it undoes nothing in general.','Singular matrices have no inverse, which is the entire point of the determinant test.','Entries can be any sign. The inverse of the identity is the identity.'],
   ...
   {q:'A dataset accidentally contains height-in-cm AND height-in-metres as two columns. Exact solving breaks because:',
    options:['The matrix is singular, since one column is a fixed multiple of the other','Heights are Gaussian, and the normal equations assume uniform features','The numbers in centimetres are too large for the solver to handle stably','Python refuses to build a matrix containing two proportional columns'],answer:0,whyWrong:['','Their distribution is irrelevant here. The problem is that one column is a fixed multiple of the other.','Magnitude is not the issue. The same failure occurs with small numbers.','Python allows it happily, which is why this bug is easy to introduce.'],
   ...
   {q:'Why prefer np.linalg.solve(A, b) over np.linalg.inv(A) @ b?',
    options:['They give unrelated answers, so the two cannot be substituted','solve accepts string input, which inv does not','solve is more accurate and faster, since forming the inverse is wasteful','inv does not exist in NumPy, so the second form fails outright'],answer:2,whyWrong:['They give the same answer in exact arithmetic. The difference is in speed and numerical error.','Neither works on strings. Both expect numeric arrays.','','It exists, and computing an explicit inverse is exactly what you want to avoid.'],
...
is why they matter to models.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The span of the vectors [1,0] and [0,1] is:',
    options:['A single line, running diagonally through the origin','The entire plane, since any point is some amount right plus some amount up','Empty, since neither vector points in a useful direction','Just those two points, and nothing in between them'],answer:1,whyWrong:['A single line is the span of one vector, or of two that are multiples of each other.','','The span always contains at least the zero vector, and here it contains everything.','The span is every combination of them, not the two vectors alone.'],
   ...
   {q:'[1,2] and [2,4] are linearly dependent. Practically, this means:',
    options:['They span the plane, since two vectors are enough to reach any point','They are both long, so neither can be ignored in the calculation','The second adds no new direction, so the span is still one line','They are perpendicular, so their dot product comes out to zero'],answer:2,whyWrong:['Two dependent vectors span a line. Spanning the plane needs two independent directions.','Length has nothing to do with it. Dependence is about direction.','','They are parallel, which is the opposite of perpendicular.'],
   ...
   {q:'If the true relationship lies OUTSIDE the span of your features, then:',
    options:['The data is corrupted and needs to be collected again','More training epochs will eventually close the gap','No linear model on those features can reach it, so you need better features','The learning rate is wrong and is stopping the fit short'],answer:2,whyWrong:['The data can be perfectly clean and the model still cannot reach the answer.','More epochs converge to the same unreachable best fit. Time does not extend the span.','','A different learning rate changes how you get to the same limit.'],
...
you: your columns are secretly redundant, go find the duplicated information.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'A square matrix is invertible if and only if:',
    options:['Every one of its entries is positive, so nothing can cancel out','Its determinant is non-zero, equivalently it has full rank','It is large enough that its rows cannot all be linearly dependent','It is symmetric, so it equals its own transpose'],answer:1,whyWrong:['Sign has nothing to do with it. Plenty of invertible matrices contain negatives.','','Size is the shape. A large matrix can be singular just as easily as a small one.','Symmetric matrices can be singular, and asymmetric ones are often invertible.'],
   ...
   {q:'How does ridge regression, (XᵀX + λI)⁻¹, help numerically?',
    options:['It deletes the correlated features before the inverse is attempted','The λI nudge restores full rank, so the inverse always exists','It scales the data up, which moves it away from the singular case','It has no effect on invertibility, only on the size of the coefficients'],answer:1,whyWrong:['Every feature is kept. Their coefficients are shrunk rather than removed.','','Nothing about the data changes. The penalty acts on the matrix being inverted.','It has a decisive effect. It is what guarantees the inverse exists.'],
...
sense once someone tells you which meaning of normal is intended.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'With 1,000 noisy data points and 2 weights, X @ w = y has:',
    options:['Infinitely many exact solutions, since a line can pass through any two points','No meaning, because the system has more equations than unknowns','Usually no exact solution, so least squares finds the closest one','Exactly one exact solution, found by inverting the matrix'],answer:2,whyWrong:['Infinitely many solutions happen when you have more weights than constraints. Here you have far more constraints.','It has a perfectly clear meaning. It is an overdetermined system.','','An exact solution would need every one of a thousand noisy points to lie on the same line.'],
   ...
   {q:'The least-squares solution leaves a residual (error) that is:',
    options:['Zero, since least squares fits the data exactly','Always positive, since the error is measured as a squared quantity','Equal to y, since the model explains none of the variation','Perpendicular to the features, so Xᵀ(y − Xw) = 0'],answer:3,whyWrong:['Zero residual would mean a perfect fit, which noisy data does not permit.','Residuals are signed. Half of them are typically negative.','That would mean the predictions were all zero, which is not what fitting produces.',''],
   ...
   {q:'"Normal" in "normal equations" refers to:',
    options:['Normalising the data before the equations are solved','Perpendicular, since the residual meets the feature directions at right angles','The normal distribution, which the noise is assumed to follow','Ordinary, in the sense of the usual rather than a special case'],answer:1,whyWrong:['Normalising the data is a preprocessing step and has nothing to do with the name.','','The Gaussian shows up elsewhere in the derivation, and it is not where the word comes from.','They are ordinary in the sense of common, and the name refers to the geometry.'],
...
one-number-per-direction problems. That is the trick, the whole trick.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'v is an eigenvector of A with eigenvalue λ means:',
    options:['A @ v = 0, so v is flattened to nothing by the matrix','v is the largest row of A, measured by its norm','λ is negative, which is what makes v an eigenvector','A @ v = λ·v, so the matrix stretches v without turning it'],answer:3,whyWrong:['That describes a vector in the null space, which is the eigenvalue-zero case rather than the general one.','An eigenvector is a direction, not one of the rows of the matrix.','Eigenvalues can be positive, negative or zero, and for a covariance matrix they are never negative.',''],
   ...
   {q:'PCA finds the main axes of a dataset by computing:',
    options:['A random projection onto a lower-dimensional subspace','The eigenvectors of the covariance matrix, the directions of most variance','The mean of each column, which marks the centre of the data','The inverse of the data matrix, which undoes the spread'],answer:1,whyWrong:['A random projection is a different technique, useful and unrelated to variance.','','The mean is subtracted first, and it is the centre rather than the direction of spread.','Nothing is inverted. The covariance matrix is decomposed.'],
   ...
   {q:'Google PageRank, eigenfaces, and vibration modes are all applications of:',
    options:['The determinant, which measures how a matrix scales area','Cosine similarity, which compares the direction of two vectors','Eigenvectors, the characteristic directions of a matrix','The L1 norm, which sums the absolute values of the entries'],answer:2,whyWrong:['The determinant is a single scalar and carries no directional information.','Cosine similarity compares two vectors and has no notion of a matrix\'s characteristic directions.','','The L1 norm measures size, and it appears in Lasso rather than here.'],
...
matrix apart into its ranked ingredients: that is the whole thing.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The outer product of a length-3 vector and a length-2 vector is:',
    options:['A single number, the sum of the pairwise products','A length-5 vector, the two joined end to end','A 3x2 matrix holding every pairwise product','Undefined, since the two lengths do not match'],answer:2,whyWrong:['A single number is the inner product, which needs matching lengths.','A length-5 vector is concatenation. The outer product builds a grid instead.','','It is well defined for any two lengths, which is precisely why it is useful.'],
   ...
   {q:'A huge ratings matrix built entirely from ONE taste factor (one outer product) has rank:',
    options:['0, since one factor cannot span any direction at all','Equal to its number of columns, since every column is present','1, since rank counts independent directions rather than size','Equal to its number of rows, since every user is represented'],answer:2,whyWrong:['Rank zero means every entry is zero.','That would be full rank, meaning every column adds a new direction.','','Row count is the shape. A million-row matrix built from one factor still has rank 1.'],
    why:'Rank counts how many rank-1 pieces you need. One factor = rank 1, even at Netflix scale.'},
   {q:'SVD is useful in ML because it:',
    options:['It makes matrices larger, so more structure becomes visible in them','It works only on square matrices, which is where the theory is cleanest','It replaces gradient descent for problems that have a closed form','It rewrites any matrix as importance-ranked rank-1 pieces'],answer:3,whyWrong:['It is used to make them smaller, by keeping only the leading pieces.','It works on any shape, which is one of its main advantages over eigen-decomposition.','It is a factorisation, not an optimiser. The two solve different problems.',''],
