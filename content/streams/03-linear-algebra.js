...
<tr><td>det(A)</td><td>the <b>determinant</b>, the volume-scaling factor (0 ⇒ no inverse)</td></tr>
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'Adding two matrices of the same shape works by:',
    options:['Multiplying rows by columns','It is not allowed','Joining them side by side','Adding matching cells, position by position'],answer:3,whyWrong:['That is matrix multiplication, and it needs the inner dimensions to agree rather than the whole shape.','Addition is defined whenever the shapes match. It is multiplication that has the awkward rules.','Joining side by side is concatenation, which changes the shape rather than combining the values.',''],
...
numbers are wrong, check whether you wanted <code>*</code> or <code>@</code>.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'[1, 2, 3] + [4, 5, 6] (as vectors) gives:',
    options:['[1,2,3,4,5,6] (joined)','[4, 10, 18]','[5, 7, 9], add matching entries','32'],answer:2,whyWrong:['That is list concatenation, which is what Python\'s + does to lists rather than to vectors.','That is the element-wise product, not the sum.','','32 is the dot product, which sums the products instead of keeping them separate.'],
...
and up it is more work by hand, but the <i>meaning</i> is identical, a volume-scaling factor.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The determinant of [[3, 1], [2, 4]] is:',
    options:['12, the product of the two entries on the main diagonal','14, the two diagonal products added together','10, the main diagonal product minus the anti-diagonal one','7, the sum of the two entries on the main diagonal'],answer:2,whyWrong:['12 is just the main diagonal product. You still have to subtract the other one.','14 adds the anti-diagonal instead of subtracting it.','','7 is the trace, 3 + 4. The trace adds the main diagonal; the determinant multiplies it and subtracts the anti-diagonal product.'],
...
un-squashed. That is the whole reason det = 0 ⇔ no inverse.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The defining property of the inverse A⁻¹ is:',
    options:['A⁻¹ is A with its rows written in the opposite order','Every square matrix has one, whatever its entries are','A⁻¹ has negative entries wherever A has positive ones','A @ A⁻¹ = I, so applying then un-applying leaves everything where it was'],answer:3,whyWrong:['Flipping is the transpose, and it undoes nothing in general.','Singular matrices have no inverse, which is the entire point of the determinant test.','Entries can be any sign. The inverse of the identity is the identity.',''],
