...
<tr><td>det(A)</td><td>the <b>determinant</b>, the volume-scaling factor (0 ⇒ no inverse)</td></tr>
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'Adding two matrices of the same shape works by:',
    options:['Multiplying each row of one by each column of the other','It is not defined, since only square matrices can be added','Joining them side by side into one wider matrix','Adding matching cells, position by position'],answer:3,whyWrong:['That is matrix multiplication, and it needs the inner dimensions to agree rather than the whole shape.','Addition is defined whenever the shapes match, square or not. It is multiplication that has the awkward rules.','Joining side by side is concatenation, which changes the shape rather than combining the values.',''],
   ...
   {q:'Why does ML use matrix operations instead of Python loops?',
    options:['Loops over numeric data are disallowed once NumPy has been imported','Matrices use noticeably less memory than the equivalent Python lists','It is a tradition inherited from the numerical computing of the 1970s','One operation applies the same work to every row, in compiled code'],answer:3,whyWrong:['Loops are legal Python. They are simply far too slow at this scale.','Memory use is comparable. The gain is that the loop moves into compiled code.','Speed is measurable and enormous. Tradition has nothing to do with it.',''],
...
numbers are wrong, check whether you wanted <code>*</code> or <code>@</code>.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'[1, 2, 3] + [4, 5, 6] (as vectors) gives:',
    options:['[1,2,3,4,5,6] (joined)','[4, 10, 18], multiply matching entries','[5, 7, 9], add matching entries','32, sum of the products'],answer:2,whyWrong:['That is list concatenation, which is what Python\'s + does to lists rather than to vectors.','That is the element-wise product, not the sum.','','32 is the dot product, which sums the products instead of keeping them separate.'],
   ...
   {q:'In NumPy, a * b (for two equal-length arrays) computes:',
    options:['Element-wise multiplication, each entry times its partner','The dot product, collapsing both arrays to a single number','An error, since the two arrays would first have to be reshaped','Matrix multiplication, treating each array as a single row'],answer:0,whyWrong:['','That is a @ b in NumPy, or np.dot.','It is perfectly legal, and it is the source of a lot of confusion precisely because it works.','Matrix multiplication is @ as well. NumPy\'s * is always element-wise.'],
   ...
   {q:'The difference between element-wise multiply and the dot product is:',
    options:['Element-wise multiplication is defined only for matrices','The dot product reverses the order of one of the vectors','They are the same operation written two different ways','The dot product also sums the products into one number'],answer:3,whyWrong:['Element-wise works on vectors, matrices and arrays of any shape.','Nothing is flipped. The transpose is a separate operation.','One returns a vector and the other returns a scalar, which is as different as two operations get.',''],
...
whole operation.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'In C = A @ B, each cell of C is:',
    options:['Always either 0 or 1, no matter what the inputs were','The dot product of a row of A with a column of B','The product of the matching cells of A and B','The sum of one row of A, taken on its own'],answer:1,whyWrong:['The entries are whatever the arithmetic gives. There is no restriction to zero and one.','','That is element-wise multiplication, which needs identical shapes and is a different operation.','Summing a row ignores B entirely, and both matrices have to contribute.'],
   ...
   {q:'X is 3×2 (3 houses, 2 features) and w has 2 weights. X @ w gives:',
    options:['2 numbers, one per feature column of X','3 numbers, one prediction per house','A 2×3 matrix, X with its shape flipped','An error, since the shapes do not line up'],answer:1,whyWrong:['One number per feature would be the weights themselves, and those are what you started with.','','The output has one entry per row of X. A 2x3 matrix is neither the right shape nor the right kind of object.','The shapes line up. 3x2 times 2x1 is valid and gives 3x1.'],
   ...
   {q:'The identity matrix I is special because:',
    options:['It sorts the rows of whatever it multiplies','It doubles every value it is applied to','It is defined only for 2x2 matrices','I @ v = v, so it changes nothing at all'],answer:3,whyWrong:['Nothing is sorted. Matrix multiplication does not reorder anything.','Doubling would be 2I. The identity has ones on the diagonal.','It exists at every size, one per square shape.',''],
...
network needs a nonlinearity between them to bend space at all (calculus/functions stream).</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'What does it mean that "a matrix transforms space"?',
    options:['It moves every point to a new location','It deletes the points that fall outside a fixed region','It measures how much space the data occupies in memory','It stores a representation of space that can be queried later'],answer:0,whyWrong:['','Points are moved rather than removed. A singular matrix collapses them onto a line, and they are still there.','Measuring size is what a determinant or a norm does.','Storage is a matter of arrays in memory, not of what the transformation means.'],
   ...
   {q:'The columns of a 2×2 matrix tell you:',
    options:['Its determinant, which is the product of the two columns','Where the basis vectors [1,0] and [0,1] land','Its inverse, read directly off the entries','How many rows the matrix has, and therefore its shape'],answer:1,whyWrong:['The determinant is a single number derived from all four entries.','','The inverse is computed from the matrix, and it is not read off the columns.','The row count is the shape, which you already know.'],
   ...
   {q:'If a matrix squashes 2-D space flat onto a single line, then:',
    options:['It becomes the identity, since the line it maps onto is its own axis','Area collapses to zero, so the determinant is zero and there is no inverse','Its determinant is huge, because the whole plane is compressed into one place','Nothing special happens, since the matrix still maps every point somewhere'],answer:1,whyWrong:['The identity leaves everything where it is, which is as far from collapse as you can get.','','A huge determinant means a large expansion. Collapse is the opposite, and it gives zero.','It is one of the most consequential things that can happen. The transformation stops being reversible.'],
...
decision, not a formality.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The distance between two data points a and b, as vectors, is:',
    options:['Their average, taken entry by entry','The norm of their difference','Their dot product, a · b','The number of entries where they match'],answer:1,whyWrong:['The average gives you a point between them, not how far apart they are.','','The dot product measures alignment. Two identical points have a large dot product and zero distance.','Counting matches is a similarity measure for categorical data, and it ignores magnitude entirely.'],
   ...
   {q:'The L1 norm of [3, −4] and the L2 norm of [3, −4] are:',
    options:['7 and 5','1 and 25','5 and 7','Both 7'],answer:0,whyWrong:['','1 and 25 are the differences and the sum of squares, neither of which is a norm.','Those are the wrong way round and the first is wrong too. L1 is 3 plus 4, L2 is the square root of 9 plus 16.','They differ. Seven is L1 and five is L2.'],
...
and up it is more work by hand, but the <i>meaning</i> is identical, a volume-scaling factor.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The determinant of [[3, 1], [2, 4]] is:',
    options:['12, the product of the two entries along the main diagonal, 3 and 4','14, the two diagonal products added together','10, the main diagonal product minus the anti-diagonal one','7, the sum of the two entries on the main diagonal'],answer:2,whyWrong:['12 is just the main diagonal product. You still have to subtract the other one.','14 adds the anti-diagonal instead of subtracting it.','','7 is the trace, 3 + 4. The trace adds the main diagonal; the determinant multiplies it and subtracts the anti-diagonal product.'],
    why:'2×2 determinant is ad − bc = (3)(4) − (1)(2) = 12 − 2 = 10.'},
   {q:'Geometrically, a determinant of 6 means the matrix:',
    options:['Has 6 somewhere among its entries','Adds 6 to each entry of the vector it is applied to','Scales every area or volume by a factor of 6','Has 6 rows, and therefore 6 dimensions'],answer:2,whyWrong:['The entries and the determinant are different things. A matrix with no 6 in it can have determinant 6.','Nothing is added. The determinant is a multiplicative scale factor.','','The row count is the shape. The determinant is computed from the values.'],
   ...
   {q:'A determinant of 0 tells you the matrix:',
    options:['Collapses space onto a lower dimension, so it has no inverse','Is the identity, which leaves every vector exactly where it started','Is very large, since a determinant of zero indicates numerical overflow','Doubles every area it touches, in the way a determinant of 2 would'],answer:0,whyWrong:['','The identity has determinant 1 and changes nothing.','Size is the shape. A large matrix can have any determinant, including zero.','Doubling would be a determinant of 2 in one dimension, or 4 in two.'],
...
un-squashed. That is the whole reason det = 0 ⇔ no inverse.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The defining property of the inverse A⁻¹ is:',
    options:['A⁻¹ is A with its rows written in the opposite order','Every square matrix has one, whatever its entries are','A⁻¹ has negative entries wherever A has positive ones','A @ A⁻¹ = I, the identity matrix'],answer:3,whyWrong:['Flipping is the transpose, and it undoes nothing in general.','Singular matrices have no inverse, which is the entire point of the determinant test.','Entries can be any sign. The inverse of the identity is the identity.',''],
    why:'Inverse = undo, and "undone" means the identity: the do-nothing matrix from lesson 2.'},
   {q:'A dataset accidentally contains height-in-cm AND height-in-meters as two columns. Exact solving breaks because:',
    options:['The matrix is singular, since one column is a fixed multiple of the other','The numbers in centimeters are too large for the solver to handle them stably','Heights are Gaussian, and the normal equations assume uniform features','Python refuses to build a matrix containing two proportional columns'],answer:0,whyWrong:['','Magnitude is not the issue. The same failure occurs with small numbers.','Their distribution is irrelevant here. The problem is that one column is a fixed multiple of the other.','Python allows it happily, which is why this bug is easy to introduce.'],
   ...
   {q:'Why prefer np.linalg.solve(A, b) over np.linalg.inv(A) @ b?',
    options:['solve accepts string input, which inv does not','solve is more accurate and faster','They give unrelated answers, so the two cannot be substituted','inv does not exist in NumPy, so the second form fails outright'],answer:1,whyWrong:['Neither works on strings. Both expect numeric arrays.','','They give the same answer in exact arithmetic. The difference is in speed and numerical error.','It exists, and computing an explicit inverse is exactly what you want to avoid.'],
...
is why they matter to models.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The span of the vectors [1,0] and [0,1] is:',
    options:['The entire plane, since any point is reachable','A single line through the origin','Empty, since neither vector points anywhere useful','Just those two points and nothing else'],answer:0,whyWrong:['','A single line is the span of one vector, or of two that are multiples of each other.','The span always contains at least the zero vector, and here it contains everything.','The span is every combination of them, not the two vectors alone.'],
   ...
   {q:'[1,2] and [2,4] are linearly dependent. Practically, this means:',
    options:['They are perpendicular, so their dot product comes out to zero','The second adds no new direction, so the span is still one line','They span the plane, since two vectors are enough to reach any point','They are both long, so neither can be ignored in the calculation'],answer:1,whyWrong:['They are parallel, which is the opposite of perpendicular.','','Two dependent vectors span a line. Spanning the plane needs two independent directions.','Length has nothing to do with it. Dependence is about direction.'],
   ...
   {q:'If the true relationship lies OUTSIDE the span of your features, then:',
    options:['The learning rate is wrong and stops the fit short','No linear model on those features can reach it','More training epochs will eventually close the gap','The data is corrupted and needs collecting again'],answer:1,whyWrong:['A different learning rate changes how you get to the same limit.','','More epochs converge to the same unreachable best fit. Time does not extend the span.','The data can be perfectly clean and the model still cannot reach the answer.'],
...
sense once someone tells you which meaning of normal is intended.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'With 1,000 noisy data points and 2 weights, X @ w = y has:',
    options:['Exactly one exact solution, found by inverting the matrix','Infinitely many exact solutions, since a line can pass through any two points','Usually no exact solution, so least squares finds the closest one','No meaning, because the system has more equations than unknowns'],answer:2,whyWrong:['An exact solution would need every one of a thousand noisy points to lie on the same line.','Infinitely many solutions happen when you have more weights than constraints. Here you have far more constraints.','','It has a perfectly clear meaning. It is an overdetermined system.'],
   ...
   {q:'The least-squares solution leaves a residual (error) that is:',
    options:['Equal to y, since the model explains none of the variation','Zero, since least squares fits the data exactly','Perpendicular to the features, so Xᵀ(y − Xw) = 0','Always positive, since the error is measured as a squared quantity'],answer:2,whyWrong:['That would mean the predictions were all zero, which is not what fitting produces.','Zero residual would mean a perfect fit, which noisy data does not permit.','','Residuals are signed. Half of them are typically negative.'],
   ...
   {q:'"Normal" in "normal equations" refers to:',
    options:['Normalizing the data before the equations are solved','Perpendicular, in the geometric sense of the word','Ordinary, in the sense of the usual rather than a special case','The normal distribution, which the noise is assumed to follow'],answer:1,whyWrong:['Normalizing the data is a preprocessing step and has nothing to do with the name.','','They are ordinary in the sense of common, and the name refers to the geometry.','The Gaussian shows up elsewhere in the derivation, and it is not where the word comes from.'],
...
one-number-per-direction problems. That is the trick, the whole trick.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'v is an eigenvector of A with eigenvalue λ means:',
    options:['v is the largest row of A, measured by its norm','λ is negative, and a negative λ is what makes v an eigenvector','A @ v = 0, so v is flattened to nothing by the matrix','A @ v = λ·v, so the matrix stretches v without turning it'],answer:3,whyWrong:['An eigenvector is a direction, not one of the rows of the matrix.','Eigenvalues can be positive, negative or zero, and for a covariance matrix they are never negative.','That describes a vector in the null space, which is the eigenvalue-zero case rather than the general one.',''],
   ...
   {q:'PCA finds the main axes of a dataset by computing:',
    options:['The eigenvectors of the covariance matrix','The mean of each column, which marks the center of the data','A random projection onto a lower-dimensional subspace','The inverse of the data matrix, which undoes the spread'],answer:0,whyWrong:['','The mean is subtracted first, and it is the center rather than the direction of spread.','A random projection is a different technique, useful and unrelated to variance.','Nothing is inverted. The covariance matrix is decomposed.'],
   ...
   {q:'Google PageRank, eigenfaces, and vibration modes are all applications of:',
    options:['Cosine similarity, which compares the direction of two vectors','Eigenvectors, the characteristic directions of a matrix','The L1 norm, which sums the absolute values of the entries','The determinant, which measures how a matrix scales area'],answer:1,whyWrong:['Cosine similarity compares two vectors and has no notion of a matrix\'s characteristic directions.','','The L1 norm measures size, and it appears in Lasso rather than here.','The determinant is a single scalar and carries no directional information.'],
...
matrix apart into its ranked ingredients: that is the whole thing.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'The outer product of a length-3 vector and a length-2 vector is:',
    options:['A single number, the sum of the pairwise products','Undefined, since the two lengths do not match','A 3x2 matrix holding every pairwise product','A length-5 vector, the two joined end to end'],answer:2,whyWrong:['A single number is the inner product, which needs matching lengths.','It is well defined for any two lengths, which is precisely why it is useful.','','A length-5 vector is concatenation. The outer product builds a grid instead.'],
