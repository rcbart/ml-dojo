...
one rule true.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'2² · 2³ equals:',
    options:['2⁶ = 64, multiplying the exponents together','2⁻¹, since combining powers subtracts the exponents rather than adding them','2⁵ = 32, since multiplying powers adds the exponents','4⁵, multiplying both the bases and the exponents'],answer:2,whyWrong:['That adds the exponents to 6 by mistake. Two plus three is five.','Subtracting exponents is what division does. Multiplying powers of the same base adds them.','','The base stays 2. It is the exponents that add, not the bases that multiply.'],
    why:'xᵃ·xᵇ = xᵃ⁺ᵇ, so 2²·2³ = 2⁵ = 32. This add-when-you-multiply rule is exactly what logs mirror.'},
   {q:'A negative exponent, like 2⁻¹, means:',
    options:['A negative number, since the minus sign carries through to the result','An undefined expression, in the way that 1/0 is undefined','Subtraction, so the expression means 2 minus 1','The reciprocal: 1/2¹ = 0.5'],answer:3,whyWrong:['2 to the power minus one is 0.5, which is positive. The sign is on the exponent.','It is perfectly well defined for any nonzero base. 2⁻¹ is simply 0.5.','That would give 1 rather than 0.5. Subtracting exponents is what division does, and a negative exponent is its result.',''],
    why:'x⁻ⁿ = 1/xⁿ. Negative exponents flip to the denominator; they are not negative numbers.'},
   {q:'x^(1/2) is another way of writing:',
    options:['the square root of x','x times 0.5, since the exponent scales the base','2ˣ, with the base and the exponent swapped round','x divided by 2, since the fraction acts as a divisor'],answer:0,whyWrong:['','Exponents do not scale the base. At x = 9 the root is 3, while 9 times 0.5 is 4.5.','That puts x in the exponent, which is a completely different function.','The fraction sits in the exponent, not outside it. At x = 16 the root is 4 and half of x is 8.'],
...
<tr><td>bˣ</td><td>b raised to the power x (any base)</td></tr>
 ...
 quiz:{title:'Quick check',questions:[
   {q:'log10(1000) equals 3 because:',
    options:['1000 minus 997, which happens to come out at 3','3 is the power you raise 10 to in order to get 1000','It is a coincidence of the numbers chosen','1000 divided by 3, rounded to the nearest whole number'],answer:1,whyWrong:['That is arithmetic on the number itself and has nothing to do with powers.','','It is a definition, and it holds for every base and every number.','Division is not what a logarithm does.'],
   ...
   {q:'The property log(a * b) = log(a) + log(b) means:',
    options:['Logarithms are only defined for whole numbers','Logarithms happen to be additive, with no deeper reason behind it','The rule holds only when a and b are equal to each other','A logarithm converts multiplication into addition'],answer:3,whyWrong:['They work on any positive real number.','It follows directly from the rule for adding exponents.','The rule holds for any a and b, equal or not.',''],
   ...
   {q:'In Python, math.log(x) (no base given) computes:',
    options:['The logarithm to base 2, the one used for bits','The number of digits in x when written out in full','The natural log, base e, which is the one ML uses everywhere','The logarithm to base 10, the one used for orders of magnitude'],answer:2,whyWrong:['Base 2 is math.log2.','Counting digits is closer to log base 10 plus one, and it is not what the function does.','','Base 10 is math.log10.'],
...
differentiable, and information-meaningful, all at once. That is why the log is everywhere.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'Why does ML maximize the LOG-likelihood instead of the likelihood itself?',
    options:['It is required by the definition of a probability distribution','Products of many probabilities underflow, and logs turn them into stable sums','Logs produce a larger number, which gradient-based optimizers converge on more readily','The likelihood itself cannot be computed for most realistic models'],answer:1,whyWrong:['No law is involved. The reason is numerical.','','Log-likelihood is usually a large negative number, so it is smaller rather than bigger.','It can be computed, and computing it is exactly what underflows.'],
   ...
   {q:'Taking the log before maximizing is safe because a logarithm is:',
    options:['Linear, so it can be moved through sums and constants freely','Monotonic, so whatever maximizes f also maximizes log f','Always positive, which keeps the objective well behaved','Faster to compute than the product it replaces'],answer:1,whyWrong:['It is decidedly nonlinear, which is why it converts products into sums.','','It is negative for inputs below one, and probabilities are all below one.','Speed is beside the point. It would not matter if the answer changed.'],
   ...
   {q:'Defining the "surprise" of an event as log(1/p) leads directly to:',
    options:['Matrix rank, which counts the independent directions in a dataset','The arithmetic mean, once the surprises are averaged in the usual way','Entropy and cross-entropy, the measures behind classification loss','The determinant, which measures how a matrix scales area'],answer:2,whyWrong:['Rank counts independent directions in a matrix.','The mean is an average of values. Surprise is averaged to give entropy, which is the next step.','','The determinant belongs to linear algebra and has no information-theoretic reading.'],
