...
one rule true.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'2² · 2³ equals:',
    options:['4⁵, multiplying both the bases and the exponents','2⁶ = 64, multiplying the exponents together','2⁻¹, subtracting the exponents rather than adding','2⁵ = 32, since multiplying powers adds the exponents'],answer:3,whyWrong:['The base stays 2. It is the exponents that add, not the bases that multiply.','That adds the exponents to 6 by mistake. Two plus three is five.','A negative exponent comes from division, not multiplication.',''],
    why:'xᵃ·xᵇ = xᵃ⁺ᵇ, so 2²·2³ = 2⁵ = 32. This add-when-you-multiply rule is exactly what logs mirror.'},
   {q:'A negative exponent, like 2⁻¹, means:',
    options:['The reciprocal: 1/2¹ = 0.5','Subtraction','An error','A negative number'],answer:0,whyWrong:['','Subtraction of exponents is what division does. A negative exponent is the result, not the operation.','It is perfectly well defined for any nonzero base.','2 to the power minus one is 0.5, which is positive. The sign is on the exponent.'],
    why:'x⁻ⁿ = 1/xⁿ. Negative exponents flip to the denominator; they are not negative numbers.'},
   {q:'x^(1/2) is another way of writing:',
    options:['the square root of x','x divided by 2','x times 0.5','2ˣ'],answer:0,whyWrong:['','Dividing by 2 is x/2, which is not a root.','Multiplying by 0.5 is the same as dividing by 2, and it is again not a root.','That puts x in the exponent, which is a completely different function.'],
...
<tr><td>bˣ</td><td>b raised to the power x (any base)</td></tr>
 ...
 quiz:{title:'Quick check',questions:[
   {q:'log10(1000) equals 3 because:',
    options:['1000 minus 997, which happens to come out at 3','It is a coincidence of the numbers chosen','3 is the power you raise 10 to in order to get 1000','1000 divided by 3, rounded to the nearest whole number'],answer:2,whyWrong:['That is arithmetic on the number itself and has nothing to do with powers.','It is a definition, and it holds for every base and every number.','','Division is not what a logarithm does.'],
   ...
   {q:'The property log(a * b) = log(a) + log(b) means:',
    options:['A logarithm converts multiplication into addition','Logarithms are only defined for whole numbers','The rule holds only when a and b are equal to each other','Logarithms happen to be additive, with no deeper reason behind it'],answer:0,whyWrong:['','They work on any positive real number.','The rule holds for any a and b, equal or not.','It follows directly from the rule for adding exponents.'],
   ...
   {q:'In Python, math.log(x) (no base given) computes:',
    options:['The number of digits in x when written out in full','The logarithm to base 10, the one used for orders of magnitude','The logarithm to base 2, the one used for bits','The natural log, base e, which is the one ML uses everywhere'],answer:3,whyWrong:['Counting digits is closer to log base 10 plus one, and it is not what the function does.','Base 10 is math.log10.','Base 2 is math.log2.',''],
...
differentiable, and information-meaningful, all at once. That is why the log is everywhere.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'Why does ML maximize the LOG-likelihood instead of the likelihood itself?',
    options:['It is required by the definition of a probability distribution','The likelihood itself cannot be computed for most realistic models','Products of many probabilities underflow, and logs turn them into stable sums','Logs produce a larger number, which optimisers converge on more readily'],answer:2,whyWrong:['No law is involved. The reason is numerical.','It can be computed, and computing it is exactly what underflows.','','Log-likelihood is usually a large negative number, so it is smaller rather than bigger.'],
   ...
   {q:'Taking the log before maximizing is safe because a logarithm is:',
    options:['Faster to compute than the product it replaces','Linear, so it can be moved through sums and constants freely','Monotonic, so whatever maximises f also maximises log f','Always positive, which keeps the objective well behaved'],answer:2,whyWrong:['Speed is beside the point. It would not matter if the answer changed.','It is decidedly nonlinear, which is why it converts products into sums.','','It is negative for inputs below one, and probabilities are all below one.'],
   ...
   {q:'Defining the "surprise" of an event as log(1/p) leads directly to:',
    options:['The determinant, which measures how a matrix scales area','The arithmetic mean, once the surprises are averaged in the usual way','Entropy and cross-entropy, the measures behind classification loss','Matrix rank, which counts the independent directions in a dataset'],answer:2,whyWrong:['The determinant belongs to linear algebra and has no information-theoretic reading.','The mean is an average of values. Surprise is averaged to give entropy, which is the next step.','','Rank counts independent directions in a matrix.'],
