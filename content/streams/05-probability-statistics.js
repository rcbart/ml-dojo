...
settle. That habit, <b>when unsure, simulate</b>, is how we'll check every formula in this
stream.</p></div>
...
tomorrow" describes your confidence, not a repeat count, since tomorrow happens once.
...
One collision to keep straight: σ (sigma, lower-case) is standard deviation here, while Σ
(Sigma, capital) was summation in the notation stream. Same letter, different jobs.
<b>MLE</b> (maximum likelihood estimation) picks the parameter values that make the data
you actually saw the most probable; it gets its own lesson.</div>
...
(1+2+…+6)/6 = <b>3.5</b>, a value it never shows, but where the average settles.
The <b>variance</b> measures <i>spread</i>: the average of the <b>squared</b> distances from
the mean. Squared for the same reason as in <b>MSE</b> (mean squared error, the loss that averages
a model's squared misses): over- and under-shoots don't cancel,
and big misses count more. A die's variance is <b>35/12 ≈ 2.92</b>, and your
...
everyday sense. A die never rolls 3.5. It's the long-run average, and the name stuck
(from 17th-century gambling math). Same for "variance": it's "average squared distance
...
billionaire in a room of ten people makes the <i>average</i> wealth huge while nine people
...
median = 2, mode = 2. When mean and median diverge, your data is <b>skewed</b>, and
the mean is pulled toward the long tail. Squared error lands on the mean,
absolute error on the median: the MSE-vs-MAE robustness story, in statistics form.
<b>MSE</b> (mean squared error) and <b>MAE</b> (mean absolute error) are two ways to score a
model: average the squared misses, which punishes big misses hard, or average the plain
misses.</p>

<h3>Spread: range, percentiles, and the IQR</h3>
<p><b>Range</b> = max − min (crude; one outlier ruins it). Better: <b>percentiles</b>. The
...
50%, a robust measure of "how spread out" that ignores the extremes. The standard outlier rule
...
not. Real ML data always has a few garbage rows. Robust summaries tell you the truth while the
mean misleads, so box plots and medians are the data scientist's default first look.</div>`,
...
<i>count</i> outcomes, and outcomes explode fast. <b>Combinatorics</b> is counting
without listing. It is the piece behind the binomial distribution, the
...
<p>Everything in this lesson is built from one rule. <b>If one choice
...
give <code>3 × 4 = 12</code> outfits. It chains across any number of stages: multiply the
number of options at each stage. Every formula below comes from this, so if you forget one
you can rebuild it from here.</p>
...
Multiply them all. That product is the <b>factorial</b>:
<code>n! = n × (n−1) × … × 2 × 1</code>. So <code>5! = 120</code>. One convention to know:
<code>0! = 1</code> (there is one way to arrange nothing). Factorials are the raw
...
<code>n! / (n−k)!</code>. Example: how many 3-letter codes from 5 distinct
letters, no repeats? <code>5 × 4 × 3 = 60</code>.</p>

<h3>Combinations: order does NOT matter</h3>
<p>Sometimes you pick a <i>group</i> where order is irrelevant (a committee, or <i>which</i> flips
came up heads). Then use <b>combinations</b>, "<b>n choose k</b>":</p>
<div class="codeSample">C(n, k) = n! / ( k! · (n−k)! )      "n choose k"      (Python: math.comb(n, k))</div>
<p>Example: how many ways to choose 2 people from 4? <code>C(4,2) = 6</code>. The link to
probability: the number of ways to get <b>k heads in n coin flips</b> is
<code>C(n, k)</code>. That's the counting factor in the <b>binomial distribution</b> you met
by shape in the distributions lesson. That is why the middle outcomes (many ways to
arrange them) are more likely than the extremes (only one way to get all-heads).</p>

<div class="demystify"><b>Demystify "n choose k":</b> it answers "how many different
...
<div class="ground"><span class="gTag">🎯 What it does</span>
<p>A <b>distribution</b> is the full "shape of the randomness": how
probability spreads over the outcomes, beyond the average. Meet the vocabulary word by word, because two letters
...
assigns each outcome its probability: P(4) = 1/6. The bars sum to 1.</p>
...
cluster around some typical value. The fix: a <b>PDF</b> (probability
<i>density</i> function) describes how <i>dense</i> probability is near each value. Real
...
<b>standard deviation</b> (how wide the bell spreads). It is everywhere because of the
<b>central limit theorem</b> (CLT). In plain words: <i>add up many small independent effects and
the total is bell-shaped, almost no matter what the pieces look like</i>. Heights (many genes
+ many environmental nudges), measurement errors, sums of dice, all Gaussian. You'll see it
yourself in the exercise: single die rolls are flat (uniform), but the <b>sum of ten dice</b>
piles up into a bell. That's the CLT happening in your own data.</p>
...
"68% within one standard deviation" isn't a mystical rule. It's the area under the bell
...
spam filter, medical test, and recommender computes conditionals. P(spam | contains
"free money"). P(disease | positive test). P(you like this movie | you liked those).</p></div>

<h3>Computing it: narrow the world</h3>
...
then count" reading is the one to keep. In a simulation it's a filter followed by
a fraction, which is what you'll run.</p>
...
independent. The first die doesn't care what the second shows. But the first die and
<i>the sum</i> are NOT independent. Learn the first is a 3 and the odds of "sum = 8" move
from 5/36 to 1/6. Same world, new knowledge, new odds.</p>

<div class="demystify"><b>Demystify the "|":</b> the vertical bar isn't division. It reads
...
A test's accuracy P(+|sick) is NOT your risk P(sick|+); the next lesson turns
this into the famous 9%.</div>

<div class="hardidea">🧠 <b>Why ML cares so much:</b> assuming independence when it's false is
one of the great silent model-killers. Two correlated features get double-counted as if they were
separate evidence. And the one assumption that makes <b>Naive Bayes</b> (a simple classifier that treats each
feature as separate evidence and multiplies the pieces together) "naive" is
...
<p><b>Bayes' theorem</b> is the rule for <b>updating a belief when evidence arrives</b>. The
famous shocker: a disease affects 1 person in 1,000. The test is 99% accurate. You test
positive. What's the chance you're sick? Most people, including most doctors in published
studies, say ~99%. The real answer is about <b>9%</b>. Here is why, with nothing but
counting.</p></div>

<h3>Count 100,000 people</h3>
<p>Out of 100,000: about <b>100 are sick</b> (1 in 1,000), and the test catches 99 of them.
...
99/1,098 ≈ <b>9%</b>. This is the <b>base-rate fallacy</b>: the rarity of the disease (the
<i>base rate</i>) overwhelms the accuracy of the test. False positives from the huge healthy
crowd swamp the true ones.</p>

<h3>Bayes' rule itself: where it comes from</h3>
<p>The formula falls out of conditional probability in two short steps.</p>
<p><b>Step 1, conditional probability</b> (from the earlier lesson). The chance of A given B is
the joint over the condition:</p>
<div class="mathblock">P(A | B) = P(A and B) / P(B)</div>
<p><b>Step 2, the joint can be written two ways</b> (the multiplication rule). That's because "A and
...
<p>That is the whole theorem. It lets you <b>flip the condition</b>. "The chance of the
evidence given the cause" is the test's accuracy, which you know. "The chance of the cause
given the evidence" is your risk, which you want. Reversing that arrow without Bayes is the
base-rate fallacy.</p>
...
<p>The one non-obvious term is <code>P(+)</code>, the total chance of a positive. It has
...
<p>The counting answer was <code>99 / 1,098</code>. The formula <i>is</i> the counting, done
with proportions instead of a census. The denominator <code>0.01098</code> is dominated by
the <code>0.00999</code> false-positive term: the healthy crowd swamping the signal.</p>

<h3>The vocabulary, demystified</h3>
<p><b>Prior</b> = your belief <i>before</i> the evidence (1/1,000, <i>a priori</i>).
<b>Likelihood</b> = how well each hypothesis explains the evidence (sick explains a positive
at 99%; healthy at 1%). <b>Posterior</b> = the updated belief <i>after</i> the evidence (~9%,
<i>a posteriori</i>). <b>Evidence</b> = the total chance of what you saw, <code>P(+)</code>,
which normalizes the result to a real probability.</p>

<div class="demystify"><b>Demystify:</b> "Bayesian inference" sounds like a philosophy degree.
It's the counting exercise above, done with algebra. When ML papers say "we place a prior on the weights," they mean this machinery.
<b>MAP</b> (maximum a posteriori) estimation = <b>MLE</b> (maximum likelihood estimation) + prior
is where <b>regularization</b> comes from. MLE picks the weights that make the data most
probable. MAP also weighs what you believed about the weights before seeing the data.
Regularization is the penalty added to the loss so the model stays simple, and it is that
prior in disguise.</div>

<div class="hardidea">🧠 <b>Hard idea, made simple:</b> the posterior depends on BOTH the
test's accuracy AND how common the condition is. That is why screening programs for rare diseases follow a positive with a
<i>second</i>, different test. The first positive raises your prior from 0.1% to 9%, and from
there the same evidence pushes the posterior to ~91%. Today's posterior is tomorrow's
prior.</div>`,
...
<p>The idea that ties this stream to everything after it: <b>training a model
is usually maximum likelihood estimation in disguise</b>. The principle (<b>MLE</b>): among
all candidate explanations, <i>pick the one that makes the data you saw least
surprising</i>. Least-squares regression? MLE under Gaussian noise. Cross-entropy, the usual loss for
...
<code>L(p) = p⁷ · (1−p)³</code>, and take the best-scoring one. The winner is
<b>0.7</b>, the observed fraction. Intuitive result, principled route, and the route is what
generalizes to a billion-parameter model where the fraction shortcut does not exist.</p>
...
everyone maximizes the <i>log</i> of the likelihood instead. Same winner, since log is
order-preserving, but sums instead of products. This is the payoff from the
Logarithms stream: product-to-sum, monotonic, differentiable. When ML code minimizes "negative
log likelihood," you can now read it: <i>maximize the data's probability, in log form,
flipped to a minimization</i>. <b>The calculus connection:</b> "maximize" means "climb
...
(maximum a posteriori) adds your prior: maximize likelihood × prior, which is Bayes' rule from
last lesson used as a fitting principle. Seen 7 heads in 10 but strongly believe coins are
fair? MAP lands between 0.7 and 0.5. You will meet this again: <b>regularization
in ML is a prior in disguise</b>. Regularization is a penalty added to the loss so the model
stays simple; the <b>L2</b> penalty adds the sum of the squared weights, which shrinks them all.
L2 penalty = Gaussian prior on the weights.</div>`,
...
<p>In English: every value, weighted by how likely it is. It is the long-run average you
would get by sampling forever. That isn't the definition, but it's the right intuition, and the law
...
<p><b>Linearity holds whether or not X and Y are independent.</b> That's stronger than most
people expect, and it's why so many derivations are shorter than they look. LOTUS, the law of
the unconscious statistician, says you can take the expectation of a function of X without ever
working out the distribution of that function.</p>
...
in squared units. That's why the <b>standard deviation</b> <code>&radic;Var</code> exists. It's
back in the units of the data and can be compared to it.</p>
...
model is minimizing an expectation you can't compute, using the average over your data as a
stand-in. That substitution is called <b>empirical risk minimization</b>. Every gap between
training performance and real performance comes from the sample average not being the
expectation.</div>`,
...
<p>Real data has many columns. Everything from covariance matrices to <b>PCA</b> to <b>Gaussian mixtures</b> rests on how
several random variables behave together. PCA (principal component analysis) finds the
directions along which data varies most, so many columns can become a few. Gaussian mixtures
are models that say the data came from a few overlapping bell curves. There are three
distributions to keep straight.</p></div>
...
<p>"Marginalizing" means summing the other variable out. The name is literal: in old
...
factors are positive and so is the product. When one is above while the other is below,
the product is negative. Average those products and you get one number. It is positive for
...
deviations and you get <b>correlation</b>, confined to [-1, 1]:</p>
...
<b>linear</b> association. Independence implies zero covariance. The reverse is false, except for
jointly Gaussian variables, where the two do coincide. That exception is a
...
<code>a<sup>T</sup>&Sigma;a &ge; 0</code> for any vector <code>a</code>. That last fact
says the variance of any linear combination of your variables can't be negative,
and it's the property the next lesson exploits.</p>
...
anything. <b>PCA</b> (principal component analysis) finds the directions along which data
varies most, so many columns can become a few. It is the optimization that falls out of
the Gaussian's geometry. This lesson does both, and
introduces <b>directional derivatives</b> along the way, the tool that makes the PCA derivation rigorous.</p></div>
...
<p>The two factors before the exponential are a normalizing
...
in the middle, which measures distance in units of standard deviation along each
...
distance knows the difference and Euclidean distance doesn't.</p>
...
<code>&Sigma;</code> records how it was stretched and rotated.</p>

<h3>Directional derivatives, since PCA needs them</h3>
<p>You know the partial derivative
<code>&part;f/&part;x<sub>i</sub></code>: how fast <code>f</code> changes as you move along one
coordinate axis. But you can move in any direction. The
...
<p>The second equality is the useful one. To first order,
<code>f(x + hu) &asymp; f(x) + h &nabla;f(x)<sup>T</sup>u</code>, so subtracting and dividing by
<code>h</code> leaves <code>&nabla;f<sup>T</sup>u</code>.</p>
<div class="hardidea">🧠 <b>Two consequences.</b> First,
<code>&nabla;f<sup>T</sup>u = ||&nabla;f|| cos&theta;</code> for a unit <code>u</code>. So the
directional derivative is largest when <code>&theta; = 0</code>, meaning <b>the gradient points
in the direction of steepest increase</b>. That's a theorem, not a definition of the gradient,
and it's the entire justification for gradient descent stepping along
...
<h3>PCA, derived</h3>
<p>The question: <b>which single direction carries the most variance in the data?</b>
...
<p>Without the constraint you could make the objective arbitrarily large by
...
of the covariance matrix</b>, and it fell out of the optimization.</p>
...
compute <code>E[xx<sup>T</sup>]</code> instead. The first component will then point roughly at the
...
to accept. It isn't arbitrary. It comes from Claude Shannon's 1948 theory of communication, and
...
and the surprise of two independent events adds. Those three requirements force the logarithm.</p>
...
now assigning short codes to things that are not common. The average message length you
achieve is the <b>cross-entropy</b>:</p>
<div class="mathblock">H(p, q) = -&Sigma;<sub>x</sub> p(x) log q(x)</div>
<p>The weights are the true probabilities <code>p</code>, but the surprise is
...
<p><b>KL</b> is short for Kullback and Leibler, who defined it. It is the excess cost of using
<code>q</code> when the truth is <code>p</code>. It is zero
when <code>q = p</code>, and otherwise strictly positive. It is <b>not symmetric</b>:
<code>D<sub>KL</sub>(p||q) &ne; D<sub>KL</sub>(q||p)</code>, so it isn't a distance despite
...
Remember this argument. The <b>EM</b> (expectation-maximization) algorithm two lessons from
now, which fits a model with hidden variables by guessing the hidden values and refitting in
turn, is built on the same inequality.</div>

<h3>Why your classifier's loss is this</h3>
...
<p>Every term vanishes except the one for the true class. So the loss for that example is
...
<code>-log</code> of something near zero is huge. That asymmetry is deliberate, and it's why
...
<code>D<sub>KL</sub>(p||q) = H(p,q) - H(p)</code> and <code>H(p)</code> doesn't depend on your
...
descriptions, one optimization, which is why the same objective keeps appearing under different
names.</div>`,
...
variables</b>, which is what "Bayesian" means.</p></div>
...
<code>&theta;</code>. You don't get one answer. You get a distribution expressing what you
now believe and how firmly.</p>
<p>The <b>evidence</b> <code>p(D) = &int; p(D|&theta;)p(&theta;) d&theta;</code> is the
normalizing constant, and it's also the reason Bayesian methods are computationally hard. That
integral is usually impossible in closed form. Nearly all of Bayesian computation exists to dodge it. <b>MCMC</b> (Markov chain Monte Carlo)
draws samples from a distribution you can't sample directly, by a random walk that lingers in
proportion to probability. <b>Variational inference</b> swaps the true posterior
for a simpler distribution and fits that instead.</p>
...
<p>The posterior is a Beta again, with the counts added to the parameters. That is what
...
<p>Sometimes you want a single number rather than a distribution. Then take the <b>maximum a posteriori</b>
...
<p>Maximum likelihood is the same expression without the second term.
<b>The prior contributes an additive term to the objective.</b> If you've read the
<b>overfitting</b> lesson in the neural networks stream (a model memorizing its training data
and doing worse on new data) you've seen this before. A zero-mean Gaussian
prior on the weights gives <code>log p(w) = -&lambda;w<sup>T</sup>w/2 + const</code>. That is
<b>L2</b> regularization: a penalty added to the loss that shrinks every weight so the model
stays simple. A Laplace prior gives <b>L1</b>, the penalty on absolute values, which pushes
weights to exactly zero.</p>
...
small" and choosing "add λ‖w‖²" are the same act in two vocabularies. That tells you how to
design a penalty: ask what you believe about the parameters before seeing data, and take its
negative log.</div>

<h3>Where the two schools differ</h3>
...
statement about <code>&theta;</code>. There's a 95% probability it lies in this range, given the
prior and the data. The second is what people usually think the first means.</p>
<p>The trade: Bayesian methods give you uncertainty as a first-class output,
and require you to state a prior and pay for the integral. Which matters depends on whether you
need calibrated uncertainty and on how much data you have. With enough data the prior
...
<p>A <b>PMF</b> (probability mass function) gives the probability of each exact value, so <code>p(x) &ge; 0</code> and
...
<p>A <b>PDF</b> (probability density function) is not a probability. <code>p(x)</code> can exceed 1, and
<code>P(X = 3) = 0</code> for every value. Only intervals carry probability:
<code>P(a &lt; X &lt; b) = &int;<sub>a</sub><sup>b</sup> p(x)dx</code>. Density is
probability per unit of x, which is why it has units and can be large where the range is
narrow.</p>
...
start. This is right for radioactive decay and wrong for machine parts, which
wear out. When someone models time-to-failure with an exponential, this is the assumption they
are making, usually without noticing.</div>
...
many-trial limit. Exponential is gamma with &alpha; = 1. Beta is the <b>conjugate prior</b> (the prior whose
posterior stays in the same family, so updating is arithmetic) for Bernoulli and binomial; gamma is the conjugate prior for the Poisson rate. The zoo is a family
tree.</div>`,
...
<p>The Gaussian shows up in noise models, priors, initialization schemes, mixture models, and
the assumptions of half of classical statistics. Mixture models say the data came from a few
overlapping bell curves. People often assume it's chosen
because the math is easy. The easy math is a consequence, not the cause. There are four
...
<p>Take independent variables with finite mean and variance, from <i>any</i> distribution at
all. The <b>central limit theorem</b> says their standardized sum converges to a Gaussian.</p>
<div class="mathblock">( &Sigma;X<sub>i</sub> - n&mu; ) / ( &sigma;&radic;n )  &rarr;  N(0, 1)   as n &rarr; &infin;</div>
<p>This is why measurement error is usually Gaussian. It's the accumulation of many small
independent perturbations, and their sum forgets what shape each one had. It also explains where
the Gaussian is <b>not</b> appropriate. Income is closer to a product than a sum of small effects,
so it is log-normal with a long right tail. Financial returns have tails far heavier
...
<h3>Reason 2: the least presumptuous choice given only a mean and a variance</h3>
<p>Take all distributions on the real line with a given mean and variance. The Gaussian has
<b>maximum entropy</b> among them. Entropy is average surprise, so maximum entropy means assuming the least
beyond what you have specified. If all you are willing to claim is a center and a spread, any
other distribution smuggles in extra assumptions about shape.</p>

<h3>Reason 3: it is closed under the operations you perform</h3>
...
messier. Marginalize a Gaussian and you get a Gaussian, with the relevant rows and columns
...
assuming Gaussian noise</b>, whether or not you said so. <b>MSE</b> is mean squared error, the
loss that averages a model's squared misses. When your
residuals are heavy-tailed, MSE is the wrong loss, and the outliers will dominate your
...
as covered in the <b>PCA</b> (principal component analysis) lesson. PCA found the directions
along which data varies most. Two facts to carry into the next lessons. For jointly Gaussian
variables, <b>zero covariance really does imply independence</b>, which is false in general.
...
<code>z</code>. The <code>&pi;<sub>k</sub></code> are called <b>mixing coefficients</b>: the
prior probability of each component before you look at the point.</p>
<div class="hardidea">🧠 <b>Why a mixture is not "several Gaussians fitted separately".</b>
If you knew which component produced each point, this would be trivial: split the data and fit
each group. You don't know. So the parameters depend on the assignments and the assignments
depend on the parameters. That circularity is the entire difficulty, and everything in the next
...
<p><b>K-means</b> is a clustering method. It assigns each point to the nearest of K centers, then
moves each center to the middle of its points. It is the special case you get from three
restrictions. Every
<code>&Sigma;<sub>k</sub> = &sigma;&sup2;I</code> with the same <code>&sigma;</code>, all
<code>&pi;<sub>k</sub></code> equal, and responsibilities forced to 0 or 1 rather than
fractions. Those restrictions cost you real things.</p>
<p>K-means finds spherical clusters of similar size, because that is what its assumptions
encode. A <b>GMM</b> (Gaussian mixture model, the model this lesson has been building) with full
covariance can find elongated, tilted clusters of different sizes and
shapes. And soft responsibilities mean a point between two clusters is reported as ambiguous rather than arbitrarily assigned.</p>
...
variance toward zero. The density at that point goes to infinity, so the likelihood does too.
This is a real singularity, not a numerical artifact. Implementations avoid it by adding a
small constant to the diagonal of each &Sigma;. Or they place a prior on the covariances and
do <b>MAP</b> (maximum a posteriori: maximum likelihood plus that prior) instead of maximum
likelihood. If your GMM produces a component that has collapsed onto
...
need parameters. <b>EM</b> (expectation-maximization) breaks it by alternating: guess the
hidden assignments, refit the parameters to them, repeat. The reason that alternation is guaranteed to
work is a beautiful piece of mathematics built on the same Jensen inequality that gave
us <code>D<sub>KL</sub> &ge; 0</code>.</p></div>

<h3>The algorithm first</h3>
...
<p>Each is the familiar sample mean, sample covariance, and proportion. The difference is that every
...
<p>We want to maximize the log-likelihood. It contains a sum inside a
...
then apply Jensen's inequality, as in the <b>KL</b> lesson:</p>
...
<code>q</code>. It is called the <b>evidence lower bound</b>, or ELBO. The gap between the
bound and the truth is a KL divergence (KL measures how different one distribution is from another):</p>
...
never negative, <code>L</code> is always below the log-likelihood. They are equal
when the KL term is zero, which happens when
...
<br><br>
So every full iteration increases the log-likelihood or leaves it unchanged. Never decreases.</div>

<h3>What the M-step maximizes</h3>
<p>With <code>q</code> fixed, drop the terms of <code>L</code> that don't involve
<code>&theta;</code>. You're left with</p>
...
<p>This is the <b>expected complete-data log-likelihood</b>, and it's why the algorithm has its
...
<code>&mu;<sub>k</sub></code> and solving yields the weighted mean. The
...
<p>It promises the likelihood never decreases, so the algorithm converges. It doesn't
...
the best. It also converges slowly near the optimum compared to second-order methods, usually
an acceptable price for closed-form steps and no learning rate (step size) to tune.</p>

<h3>Where else you will meet it</h3>
<p>EM is a general recipe, not a <b>GMM</b> (Gaussian mixture model, the overlapping bell curves
of the last lesson) trick. It fits hidden Markov models through the
Baum-Welch algorithm. It handles missing data by treating the missing entries as latent. It
is behind probabilistic <b>PCA</b> (principal component analysis, which reduces many columns to a
few directions of greatest variation) and factor analysis, and it fits topic models. The ELBO you just
derived is the same object that <b>variational autoencoders</b> (VAEs, neural networks that
compress an input into a small random code and learn to decode it) maximize. The difference is that when
...
<p>Having done the Gaussian and the mixture properly, this lesson collects the other places the
same distribution appears. Recognizing it saves you from learning several things that are
...
Add a Gaussian prior on <code>w</code> and <b>MAP</b> (maximum a posteriori, maximum likelihood
plus that prior) becomes <b>ridge regression</b>, least squares with a penalty that shrinks the
weights. The classical
toolkit is Gaussian assumptions under different names.</p>
...
The decision boundary can be roughly right even when the probabilities are badly
calibrated.</p>
...
<b>Xavier</b> initialization uses <code>Var = 2/(n<sub>in</sub>+n<sub>out</sub>)</code>.
<b>He</b> initialization uses <code>Var = 2/n<sub>in</sub></code> for <b>ReLU</b> networks. ReLU is the
activation that passes positive values through and zeroes negative ones.</p>
...
finite set of points has a joint Gaussian distribution. The covariance between two points
is given by a kernel measuring how similar their inputs are. Because Gaussians are closed under
conditioning, observing some points gives a closed-form posterior over the rest, with calibrated
uncertainty. Excellent with little data. The cost is <code>O(n&sup3;)</code>, which is why it
isn't used with much of it.</p>

<h3>5. Variational autoencoders</h3>
<p>A <b>VAE</b> (variational autoencoder) encodes each input to a Gaussian in latent space rather
than to a point, samples from it, and decodes. The objective is the <b>ELBO</b> from the <b>EM</b>
(expectation-maximization) lesson. The ELBO (evidence lower bound) is the stand-in you push up
because the true likelihood is out of reach. Here it is reconstruction accuracy minus the KL
divergence (how far one distribution is from another) between the encoder's Gaussian and a
standard Gaussian prior. Both Gaussians are
chosen so that KL has a closed form, and so that sampling can be written as
<code>z = &mu; + &sigma;&odot;&epsilon;</code> with <code>&epsilon; ~ N(0,I)</code>. That keeps
...
about why those guesses work, when they are systematically wrong, and where a
...
<p>The <b>cumulative distribution function</b> (CDF) always exists, whether or not a density does.
It's what percentiles are read from. The median is <code>F<sup>-1</sup>(0.5)</code>. It also
gives you a way to generate samples: if <code>U</code> is uniform on [0,1], then
<code>F<sup>-1</sup>(U)</code> has distribution <code>F</code>. That's <b>inverse transform
sampling</b>, and it's how a random number generator producing uniforms becomes one producing
anything.</p>
...
<p>(<b>MSE</b> is mean squared error, the estimator's average squared miss.)
<b>Unbiased</b> means right on average across repeated samples. It doesn't mean right on your
sample. A biased estimator with low variance often beats an unbiased one with high variance,
the trade the next lesson formalizes.</p>

<h3>Where n-1 comes from, derived</h3>
<p>The sample mean is unbiased: <code>E[x&#772;] = &mu;</code>, directly by linearity. The naive
sample variance isn't. Consider dividing by <code>n</code>:</p>
<div class="mathblock">E[ (1/n)&Sigma;(x<sub>i</sub> - x&#772;)&sup2; ] = &sigma;&sup2; (n-1)/n  &lt;  &sigma;&sup2;</div>
<p>It underestimates, every time. The deviations are measured
from <code>x&#772;</code>, which was computed from the same data and therefore sits closer
...
<p>This is the theorem that says estimation is possible at all.</p>
<p><b>The central limit theorem</b> tells you how fast, and in what shape. The standard deviation
of the sample mean is <code>&sigma;/&radic;n</code>, called the <b>standard error</b>. The
...
reducing it tenfold requires a hundred times. This is why the step from 100 to
1,000 examples transforms a model and the step from 100,000 to 1,000,000 often barely moves it.
Past a point, buying more data is worse value than improving the model or the
features.</div>
...
wins over grid methods in high dimensions, where a grid is hopeless. <b>Dropout</b> (randomly switching off units in a network) at prediction time,
...
at the spread of the results. It treats your sample as a stand-in for the population, justified
by the same law of large numbers. It is also the idea behind <b>bagging</b>, training many copies of a model on resampled data
and averaging their answers, and therefore behind random forests.</p>`,
...
error splits into three parts, one of which you can't do anything about. Once you have the
...
when the training set changes. <b>Irreducible error</b> is the noise. No model, no data and
...
<p><b>Underfitting is high bias.</b> The model family can't represent the pattern, so training
error and validation error (the error on data held back from training) are both high and close
together. <b>Overfitting is high variance.</b>
...
<p>That gap now has an interpretation: <b>it is an estimate of the variance term</b>. The
floor that validation error can't go below is <code>bias&sup2; + &sigma;&sup2;</code>.</p>
...
problem, then spend everything else on controlling variance. That is modern practice, and it
explains why large models plus heavy <b>regularization</b> (penalties added to the loss that keep the
model simple) plus large data is the dominant recipe.</p>
...
they can interpolate the training data exactly. Test error often <b>falls again</b> rather
than continuing to rise. This is called <b>double descent</b>. It doesn't violate the
decomposition, which is an identity and can't be violated. In the heavily
overparameterized regime, the implicit bias of the optimizer selects among the many perfect fits,
and gradient descent happens to prefer smooth ones. What fails is the
assumption that variance must grow with parameter count.</div>
...
reduction:</p>
<p><b>Transfer learning</b> is the strongest option. Start from a model trained on a
...
lower variance. That matters most when you can least afford to waste rows on a test set.
<b>Simpler models</b> accept bias in exchange for variance, the correct trade when
<code>n</code> is small, and why linear models and gradient-boosted trees still win on
small tabular data. <b>Stronger regularization</b> and <b>ensembling</b> (averaging several models' answers) do the
same in different currencies. A <b>Bayesian treatment</b> with an informative prior is the principled version
of all of them, since the prior supplies information the data can't.</p>`,
