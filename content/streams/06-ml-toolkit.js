...
now.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   ...
   {q:'Why is looping over array elements in Python the cardinal performance sin?',
    options:['Every element makes a trip through the interpreter instead of staying in compiled code','Each iteration allocates a fresh temporary array, so memory grows with the element count','Loops over arrays are forbidden syntax once NumPy has been imported','It only matters on a GPU, where the transfer cost per element dominates'],answer:0,whyWrong:['','Memory use is comparable to the vectorized version. It is the per-element interpreter overhead that costs you.','Loops are legal Python and are the right tool for plenty of tasks.','It matters on a CPU just as much. NumPy on a CPU is already compiled code.'],
...
arrays for speed. Nothing more frightening than a spreadsheet with an API.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'df[df["age"] > 40] is:',
    options:['A file operation, reading the matching rows from disk','Boolean-mask filtering, the NumPy idea applied to labeled rows','A groupby that splits the rows into an over-40 group and an under-40 group','A syntax error, since a DataFrame cannot index itself'],answer:1,whyWrong:['Nothing is read or written. It selects rows already in memory.','','A groupby would hand you both groups. This keeps only the rows that pass the test.','It is valid pandas and one of the most common lines you will write.'],
...
says "train," picture the bowl and the walk downhill, never an incantation.</div>`,
 ...
 quiz:{title:'Quick check',questions:[
   {q:'model.fit(X, y) on a LinearRegression does what, in terms you already know?',
    options:['Sorts the rows by X so the relationship between X and y becomes visible in a plot','Downloads a pretrained model and adapts it to the data you passed','Runs a proprietary routine whose internals are not published','Solves the least-squares problem, the same normal equations you implemented'],answer:3,whyWrong:['Sorting changes the row order and nothing else. The coefficients would still be unset.','Nothing is downloaded. It computes the fit from the data you handed it.','It is open source, and it is the same normal equations you derived by hand.',''],
   ...
   {q:'Why does every sklearn model sharing fit/predict matter for learning ML?',
    options:['One interface, many models, so each new concept plugs into a familiar shape','It means every model runs the same optimizer internally, so their results are comparable','It removes the need to understand what each model is doing','It does not matter, since the concepts are what you are learning'],answer:0,whyWrong:['','A decision tree and an SVM share no optimizer at all. What is shared is the interface.','It makes the concepts easier to compare, and you still have to understand them.','It matters a great deal. It is why each new model costs you almost no new syntax.'],
