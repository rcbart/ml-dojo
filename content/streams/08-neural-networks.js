...
`,
 ...
 quiz:{title:'Quick check, XOR',questions:[
   ...
   {q:'What stopped researchers from simply stacking layers in 1969?',
    options:['The mathematics of stacking layers had not been worked out','Computers of the era could not store the weights','There was no way to train the neurons in the middle','Stacked layers were proven to be no more powerful'],answer:2,whyWrong:['Composing layers was straightforward to describe. Assigning blame to the middle ones was not.','The networks of the era were small enough to store without difficulty.','','Stacking was understood to be more powerful in principle. Training was the obstacle.'],
   ...
   {q:'What does adding a hidden layer buy you?',
    options:['Faster convergence on the same class of problems a single layer solves','Fewer weights to store and update','Boundaries built from several pieces rather than one line','Guaranteed accuracy on unseen examples'],answer:2,whyWrong:['Convergence often gets slower. What changes is which problems are solvable at all.','It adds weights rather than removing them, sometimes a great many.','','Nothing guarantees accuracy on unseen data. A hidden layer makes overfitting easier, not harder.'],
...
`,
 ...
 quiz:{title:'Quick check, layers',questions:[
   {q:'A layer is called "hidden" because:',
    options:['It is optional and can be removed safely','It only activates for certain inputs','Its weights are encrypted while training is running','Nothing outside the network observes its values'],answer:3,whyWrong:['Removing it takes you back to a linear model, which is the thing the hidden layer was added to escape.','It activates for every input. Whether the output is nonzero is a separate matter.','Nothing is encrypted. The weights sit in memory as ordinary numbers.',''],
...
`,
 ...
 quiz:{title:'Quick check, activations',questions:[
   {q:'Removing the activation functions from a deep network would:',
    options:['Make it equivalent to a single linear layer','Improve accuracy while costing more memory per layer','Leave the network unchanged in practice','Make training slower but still possible'],answer:0,whyWrong:['','Accuracy would collapse to whatever a linear model can manage, which is usually far less.','It changes a great deal. Composing linear maps gives you one linear map.','Speed is not the issue. The model becomes strictly less expressive.'],
...
`,
 ...
 quiz:{title:'Quick check, backpropagation',questions:[
   ...
   {q:'Why does the algorithm run backward from the output?',
    options:['Because the output layer holds the largest weights in the network','Because forward passes are too slow to repeat','Because the inputs are not known until the end','So each layer can reuse the sensitivity passed down to it'],answer:3,whyWrong:['Weight magnitudes vary by layer and have nothing to do with the direction of the sweep.','The forward pass runs once. It is the per-weight sensitivity that would be expensive to recompute.','The inputs are known first. It is the error that is only known at the end.',''],
   ...
   {q:'What did the 1986 paper demonstrate that mattered most?',
    options:['That hidden layers learn useful features unprompted','That networks converge faster than decision trees','That the chain rule applies to functions composed in layers','That deeper networks always beat shallow ones'],answer:0,whyWrong:['','No such comparison was the point, and decision trees were a separate line of work.','The chain rule was centuries old. Applying it to learned internal representations was the news.','Depth was not shown to always win, and it often does not.'],
...
`,
 ...
 quiz:{title:'Quick check, convolution',questions:[
   {q:'A convolution kernel is:',
    options:['A small grid of weights applied across the whole image','A compressed copy of the input image','A layer that reduces the number of channels in the input','A rule for choosing which pixels to discard'],answer:0,whyWrong:['','Nothing is copied. The kernel is learned and is far smaller than the image.','Channel reduction is a 1x1 convolution or a pooling layer. The kernel itself is just weights.','Discarding pixels is what striding and pooling do. The kernel computes, it does not select.'],
   ...
   {q:'A 3x3 kernel over 3 channels producing 64 outputs has how many parameters?',
    options:['576, ignoring the bias terms','1,792, including one bias per output','9, since the kernel is shared','192, which is 3 channels times 64 kernels'],answer:1,whyWrong:['576 is 3x3x64, which forgets the three input channels and the biases.','','Nine counts one channel of one kernel. There are three input channels and sixty-four kernels.','That multiplies channels by kernels and skips the 3x3 grid entirely. The total is 3x3x3x64 weights plus 64 biases.'],
...
`,
 ...
 quiz:{title:'Quick check, attention',questions:[
   ...
   {q:'Scores are divided by the square root of the key dimension because otherwise:',
    options:['Softmax saturates and the gradient disappears','The attention weights would stop summing to one','Negative scores would dominate the average','The values would be scaled inconsistently'],answer:0,whyWrong:['','Softmax always sums to one, at any scale. What changes is how sharply peaked it is.','Softmax handles negative scores without difficulty. Scale is the problem, not sign.','The values are untouched. The division applies to the scores, before softmax.'],
