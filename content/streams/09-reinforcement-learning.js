...
`,
 ...
 quiz:{title:'Quick check, what makes RL different',questions:[
   ...
   {q:'The credit assignment problem is:',
    options:['Deciding how to divide a single shared reward between several agents acting at the same time',
   ...
   {q:'You have a large labeled dataset for your task. Should you use RL?',
    options:[
             ...
             'Yes, because RL can optimize the metric you actually care about rather than a proxy loss','Yes, RL generalizes better than supervised learning on the same data',
 ...
 quiz:{title:'Quick check, returns and discounting',questions:[
   ...
   {q:'Writing the reward as r_{t+1} rather than r_t is meant to signal that:',
    options:['Rewards are always delayed by exactly one step in every environment',
             'Rewards and states are stored in two separate buffers whose indices are offset by one step',
 ...
 quiz:{title:'Quick check, values and Bellman',questions:[
   {q:'Q(s,a) is more useful than V(s) for acting because:',
    options:['It is a smaller table and therefore cheaper to store in memory',
             'It converges faster than V under the same iterative update rule and step size',
   ...
   {q:'Iterative policy evaluation converges from any starting values because:',
    options:['The value function is convex in the policy parameters',
             ...
             'The rewards are bounded, so the estimates are confined to a fixed interval from the start',
   ...
   {q:'V^π(s) is written with a superscript π because:',
    options:[
             'The superscript records how many iterations of the update rule have been applied',
 ...
 quiz:{title:'Quick check, optimality and dynamic programming',questions:[
   {q:'The Bellman optimality equation differs from the Bellman equation by:',
    options:[
             ...
             'Removing the discount factor entirely, since an optimal policy needs no horizon','Replacing the average over actions with a maximum, which makes it non-linear',
...
`,
 ...
 quiz:{title:'Quick check, Monte Carlo',questions:[
   {q:'Monte Carlo methods need no model because:',
    options:[
             'They only ever visit states whose dynamics are already known','They estimate the transition probabilities from visit counts before evaluating anything',
   ...
   {q:'A greedy policy combined with Monte Carlo control fails because:',
    options:[
             'Averaging returns under a policy that keeps changing mixes samples from different distributions',
 ...
 quiz:{title:'Quick check, TD learning',questions:[
   ...
   {q:'TD usually beats Monte Carlo in practice mainly because:',
    options:[
             ...
             'It does not need the Markov property to hold in the state representation you happened to choose'],answer:2,whyWrong:['Both store one value per state. Storage is identical.','Also the other way round. TD is the biased one, because its target uses an estimate.','','The other way round. TD is the method that leans on the Markov property.'],
 ...
 quiz:{title:'Quick check, SARSA and Q-learning',questions:[
   ...
   {q:'On the cliff-walking task SARSA scores better during training because:',
    options:[
             ...
             'It stops taking random actions once the value estimates have stabilized late in training',
   ...
   {q:'"Off-policy" means the algorithm can:',
    options:[
             'Switch to a new policy without discarding the value function it learned',
...
`,
 ...
 quiz:{title:'Quick check, exploration',questions:[
   {q:'Regret that grows linearly in T means:',
    options:[
             'The algorithm converges to the best arm, but more slowly than the theoretical optimum allows',
   ...
   {q:'The principle behind UCB is:',
    options:[
             'Decay the exploration rate in proportion to the total number of arms you were given',
   ...
   {q:'Random exploration fails on Montezuma\'s Revenge because:',
    options:['The reward signal is far too noisy for the value estimates to converge on anything',
...
`,
 ...
 quiz:{title:'Quick check, function approximation',questions:[
   ...
   {q:'Tabular convergence proofs stop applying under function approximation because:',
    options:[
             'Neural networks are non-convex, so gradient descent cannot be relied on to reach a global optimum',
   ...
   {q:'The main thing function approximation buys you is:',
    options:[
             'The ability to learn without any reward signal at all','A guaranteed reduction in the variance of the value estimates you get',
...
`,
 ...
 quiz:{title:'Quick check, DQN',questions:[
   {q:'Experience replay helps mainly because:',
    options:[
             'It removes the need for a discount factor by storing whole episodes from end to end',
   ...
   {q:'DQN clips rewards to −1, 0 and +1 because:',
    options:[
             'Clipping is what turns the value function into a proper probability distribution over returns',
...
`,
 ...
 quiz:{title:'Quick check, policy gradients',questions:[
   {q:'The score function trick lets you:',
    options:['Differentiate through the environment dynamics using a learned world model',
   ...
   {q:'REINFORCE in one sentence:',
    options:[
             'Follow the gradient of the squared TD error with respect to the value function parameters',
...
`,
 ...
 quiz:{title:'Quick check, actor-critic and PPO',questions:[
   {q:'The critic\'s job in an actor-critic method is to:',
    options:['Choose which action the agent takes at each step','Decide when the episode should be terminated early','Store past transitions so they can be replayed again later','Provide a low-variance advantage to weight the gradient'],answer:3,whyWrong:['That is the actor\'s job. The critic never selects anything.','Termination comes from the environment. Neither component decides it.','That is a replay buffer, and standard actor-critic methods are on-policy and do not use one.',''],
 ...
 quiz:{title:'Quick check, RLHF',questions:[
   ...
   {q:'The KL penalty against the initial model is there to:',
    options:[
             ...
             'Ensure the generated responses stay within a fixed token budget per prompt','Keep the policy where the reward model is still a trustworthy measure',
...
`,
 ...
 quiz:{title:'Quick check, limits and judgement',questions:[
   {q:'The main practical consequence of RL\'s sample inefficiency is that:',
    options:['Value functions have to be approximated rather than tabulated exactly',
   ...
   {q:'A team wants to pick which of five banner images to show each visitor, with the outcome known immediately. The right tool is:',
    options:[
             'Deep Q-learning with a large replay buffer holding past visitor sessions',
