...
{domain:'Calculus & Optimisation',icon:'📈',groups:[
 {h:'Derivatives',terms:[
 ...
 {h:'Finding the best answer',terms:[
  ...
  ['Constrained optimisation','Maximising or minimising subject to a rule you are not allowed to break, such as probabilities summing to one or a vector having unit length.'],
...
{domain:'Probability & Statistics',icon:'🎲',groups:[
 ...
 {h:'Distributions',terms:[
  ...
  ['Gaussian (normal)','The bell curve, parameterised by mean and variance. Appears everywhere because of the central limit theorem, and because it is the maximum-entropy distribution for a given mean and variance.'],
 ...
 {h:'Inference and estimation',terms:[
  ...
  ['Expectation maximisation (EM)','Alternating between guessing the latent variables given the parameters, and re-fitting the parameters given those guesses. Each round provably does not decrease the likelihood.'],
...
{domain:'Machine Learning Concepts',icon:'🤖',groups:[
 {h:'The setup',terms:[
  ...
  ['Semi-supervised learning','Using a small labelled set together with a large unlabelled one. Worth reaching for when labels are the expensive part, which is most of the time.'],
 ...
 {h:'Fit and generalisation',terms:[
  ...
  ['Regularisation','Anything that penalises complexity to improve generalisation. L2 shrinks weights toward zero, L1 drives some of them exactly to zero.'],
  ['Generalisation','Performance on data the model has not seen. The only thing that matters, and the only thing training error does not measure.'],
 ...
 {h:'Measuring it',terms:[
  ['Loss function','The number training minimises. Mean squared error for regression, cross-entropy for classification. It is the specification, so choose it deliberately.'],
...
{domain:'Classic Models',icon:'🧰',groups:[
 {h:'Regression',terms:[
  ['Linear regression','Fits a straight line or hyperplane by minimising squared error. Has a closed-form solution and remains the right answer more often than people expect.'],
 ...
 {h:'Classifiers',terms:[
  ['k-nearest neighbours (k-NN)','Classify by a vote of the k closest training points. No training phase at all, and it degrades badly as dimensions grow.'],
 ...
 {h:'Unsupervised',terms:[
  ['k-means','Alternates assigning points to the nearest centre and moving each centre to the mean of its points. Fast, and it assumes clusters are round and similarly sized.'],
  ...
  ['Principal component analysis (PCA)','Finds the directions of greatest variance, which turn out to be the eigenvectors of the covariance matrix. The constrained-optimisation problem behind it is solved with a Lagrange multiplier.'],
...
{domain:'Neural Networks',icon:'🧠',groups:[
 ...
 {h:'Training',terms:[
  ...
  ['Adam','Per-parameter step sizes from running averages of the gradient and its square, with a bias correction for the first few steps. The usual default optimiser.'],
  ['Vanishing and exploding gradients','Gradients shrinking or growing multiplicatively through depth. The reason for ReLU, careful initialisation, normalisation layers and residual connections.'],
  ['Initialisation','The starting values of the weights. Scaled to keep signal variance stable through depth, and getting it wrong stops training before it starts.'],
  ['Batch normalisation','Normalising activations within a batch, then rescaling with learned parameters. Speeds up training and adds a little regularisation as a side effect.'],
  ['Dropout','Randomly zeroing units during training so the network cannot depend on any one of them. Off at inference.'],
  ['Early stopping','Halting when validation error stops improving. The simplest regularisation there is, and often the most effective.'],
 ]},
...
{domain:'Reinforcement Learning',icon:'🕹',groups:[
 {h:'The setup',terms:[
  ...
  ['Policy','The agent\'s behaviour, a rule from states to actions. Deterministic or a distribution over actions. It is the thing you are actually trying to learn.'],
  ...
  ['Return','The discounted sum of future rewards from here on. What the agent maximises, and not the same thing as the next reward.'],
  ['Discount factor (γ)','How much future reward is worth now. Sets the planning horizon at roughly 1/(1−γ) steps, and it is a modelling decision rather than a knob.'],
 ...
 {h:'Modern methods',terms:[
  ...
  ['Multi-armed bandit','One state, several actions, no transitions. The cleanest setting for studying exploration, and the right model for a great many problems mislabelled as RL.'],
  ...
  ['Policy gradient','Optimising the policy parameters directly by gradient ascent on expected return. The score function identity is what makes the gradient estimable by sampling.'],
  ...
  ['RLHF','Learning a reward model from human preference rankings, then optimising the policy against it with PPO and a KL penalty toward the starting model.'],
  ['Reward hacking','Finding behaviour that scores well under the reward function and is not what anyone meant. The reward function is the specification, and a capable optimiser will find where it comes apart from the intent.'],
 ]},
]},
